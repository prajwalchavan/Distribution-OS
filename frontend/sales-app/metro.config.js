 ...
 * 3b. **`.wasm` is an asset.**
 *
 * This is the first app to DECLARE `expo-sqlite` (docs/18: the first field app must, or autolinking
 * never links it and the offline store silently falls back to memory). Its web build is wa-sqlite,
 * and `expo-sqlite/web/worker.ts` does `import wasmModule from './wa-sqlite/wa-sqlite.wasm'` — which
 ...
 * as a URL, which is exactly what wa-sqlite wants. Without it, no app that declares `expo-sqlite`
 * can produce a web build at all, so this belongs in every field app (and in the template).
 */
