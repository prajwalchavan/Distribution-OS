/**
 * DOS-167 — what the sales app says when a rep signs out, or switches distributor, with changes that
 ...
 *
 * Pure rules plus a read of the sheet's source, like `order-entry-layout.test.ts`: importing a screen
 * in Node pulls in `react-native`, which does not resolve outside Metro. `@types/node` is deliberately
 * absent from an app, so the two Node functions are imported through non-literal specifiers.
 */
...
describe('DOS-167 the sales leave sheet', () => {
  it('DOS-167 leaveSentence names the count, the person and the rule for sign-out and for a switch', async () => {
    ...
    expect(queued.body).toBe(
      'They stay on this phone for Rahul Deshmukh only and go the next time Rahul Deshmukh signs in here. Nobody else can see them.',
    )

    // Two orders the office refused, nothing queued.
    ...
    expect(refused.body).toContain(RAHUL)
    ...
    expect(switching.body).not.toContain(RAHUL)
  ...
  it("DOS-167 the sheet counts one change in the singular, tells the truth about refusals, and keeps every button under the kit's 20 characters", () => {
    // One order taken in a dead spot.
  ...
  /*
   * Ruling 2 (u), merge review minor 2. `end()` keeps the file by itself when its re-count finds something waiting — an
   * order in hand at the tap — and the drafts of that rep stay with it. The other distributorships are swept on every
   * sign-out, a kept one included: the sweep deletes only files with nothing unsent.
   */
  it('DOS-167 a file kept by the re-count keeps the drafts, and the sweep runs on every sign-out', async () => {
    ...
    // One tap with nothing counted, and an order in hand landed: the engine kept the file.
    ...
    // Nothing waited and the file went: the drafts go with it.
    ...
    expect({ recount: recount.calls, keep: keep.calls, gone: gone.calls }).toEqual({
      ...
      gone: ['signOutOnDevice', 'end keepQueue=false', 'sweep', 'forgetDrafts', 'revoke'],
    })
  })
  ...
  it('DOS-167 leaving decides on what waits in the file once it is open, clears the session on the phone before it ends the engine and never deletes what it did not count', async () => {
    ...
    // (file deleted), the person's other files are swept, this rep's drafts are forgotten, and the revoke goes last.
    const clean = phone({})
    ...
    // "Sign out, keep here" keeps the queue and the drafts, and sweeps the other distributorships as every
    // sign-out does (ruling 2 (u)); "Switch anyway" only switches.
    const keep = phone({})
    await leaveNow({ mode: 'signOut' }, true, keep.steps)
  ...
  /*
   * Addendum (z1). In the delivery and warehouse apps "Sign out" on the Settings screen called `useSession().signOut()`
   * itself: no sheet, no `end()`, the read set left on the phone, the other distributorships never swept, the session
   * cleared only once the server's revoke had answered — and on a browser that keeps nothing, the queue thrown away.
   * This app has no second sign-out button; the guard keeps it so. Every sign-out button takes the leave flow of
   * `app/sales/_layout.tsx`, through `useLeave()`. The layout's one other sign-out was the wrong-role screen's, where no
   * device store was ever opened — and that screen is GONE with the one-app merge (docs/31 §1.3: the person elects a role
   * at sign-in, so there is no wrong app to be in). The subtraction below is kept as written: with no `<WrongRole/>` in
   * the layout it takes nothing away, and the layout must still call `signOut()` zero times.
   */
  it('DOS-167 every sign-out button in the app goes through the leave flow', async () => {
    ...
    const { fileURLToPath } = (await import(NODE_URL)) as NodeUrl
    const app = fileURLToPath(new URL('../../../../app/sales', import.meta.url))
