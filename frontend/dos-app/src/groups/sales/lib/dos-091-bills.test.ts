...
describe('DOS-091 a bill the rep can open, with copy that reads the sign of ageDays', () => {
  ...
  it('DOS-091 the bills tab opens a bill detail route that reads the invoice and its PDF', async () => {
    ...
    const detail = await source('../../../../app/sales/bills/[id].tsx')
    const strings = await source('../strings.ts')

    expect({
      // Every row is a tap target, online rows and the phone's own copy alike. The move is `go.push`
      // since the one-app merge (docs/31 §1.1): the group's base is added once, by the helper.
      opensOnline: /onPress=\{\(\) => \{\s*go\.push\(`\/bills\/\$\{bill\.id\}`\)/.test(card),
      opensLocal: /onPress=\{\(\) => \{\s*go\.push\(`\/bills\/\$\{bill\.id\}`\)/g.test(card),
      taps: (card.match(/go\.push\(`\/bills\/\$\{bill\.id\}`\)/g) ?? []).length,
