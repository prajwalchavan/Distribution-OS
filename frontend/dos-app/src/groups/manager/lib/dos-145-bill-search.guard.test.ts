...
describe('DOS-145 the manager opens a bill from search', () => {
  it('DOS-145: a bill hit in the header search lands on the Billing desk, on Bills issued, with that bill open', async () => {
    const code = withoutComments(await read('../../../../app/manager/_layout.tsx'))
    ...
    // The one app gives every group a visible segment, so a hit's href is built by the kit's
    // `routeFor(group, path)` rather than written whole (docs/31 ruling Q1). What is asserted is
    // still the path this desk sends the reader to; the group base is `route-for.test.tsx`'s job.
    const href = /href: (?:routeFor\(\s*GROUP,\s*)?`([^`]*)`/.exec(billHit ?? '')?.[1] ?? ''
  ...
  it('DOS-145: the Billing desk opens on Bills issued when the url carries a bill, and opens that bill', async () => {
    const code = withoutComments(await read('../../../../app/manager/billing/index.tsx'))
