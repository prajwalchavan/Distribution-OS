/**
 * DOS-167 — what the warehouse app says when a godown hand signs out, or switches distributor, with
 * picks or counts that have not reached the office.
 *
...
describe('DOS-167 the warehouse leave sheet', () => {
  it('DOS-167 leaveSentence names the count, the person and the rule for sign-out and for a switch', async () => {
    // Three picks confirmed in the back of the shed with no signal.
    ...
    expect(queued.body).toBe(
      'They stay on this phone for Dinesh Patil only and go the next time Dinesh Patil signs in here. Nobody else can see them.',
    )

    // Two picks the office refused, nothing queued.
    ...
    expect(refused.body).toContain(DINESH)
    ...
    expect(switching.body).not.toContain(DINESH)
  ...
  it('DOS-167 leaving decides on what waits in the file once it is open, clears the session on the phone before it ends the engine and never deletes what it did not count', async () => {
    ...
    // The godown hand kept a pick on this phone, signed in again and tapped Sign out while the file was
    // still opening. The count is waited for, and the sheet asks: nothing is ended and nobody is signed
    // out.
  ...
  it('DOS-167 every sign-out button in the app goes through the leave flow', async () => {
    ...
    const { fileURLToPath } = (await import(NODE_URL)) as NodeUrl
    const app = fileURLToPath(new URL('../../../../app/warehouse', import.meta.url))
