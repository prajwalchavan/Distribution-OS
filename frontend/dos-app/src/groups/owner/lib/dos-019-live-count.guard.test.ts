 ...
 *
 * O1 read `owner_summary.pendingApprovals` in both places (`app/owner/index.tsx`,
 * `app/owner/_layout.tsx`). That figure is written by the worker every 15 minutes and counts approvals
...
describe('O1 Today: the count of decisions waiting', () => {
  it('DOS-019: the heading counts the live rows, not the 15-minute rollup', async () => {
    const code = withoutComments(await read('../../../../app/owner/index.tsx'))
  ...
  it('DOS-019: the rail badge counts the same rows, under the same query keys', async () => {
    const layout = withoutComments(await read('../../../../app/owner/_layout.tsx'))
    const today = withoutComments(await read('../../../../app/owner/index.tsx'))
