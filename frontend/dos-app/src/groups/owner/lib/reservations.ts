 ...
 *
 * The order panel printed `items.length` under "Stock held" — the number of LOTS the godown reserved
 * from, which an owner reads as a quantity. Holds are per lot, so one case can be five rows; the
 * figure that answers "what has this order taken out of stock" is the sum of `qtyPcs`.
 *
 ...
 * and this file is tested without one.
 */
