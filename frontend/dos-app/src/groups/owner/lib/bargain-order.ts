/**
 * What a rate request's order id MEANS in the owner's Approvals (DOS-090).
 *
 * The owner's twin of the manager's `src/lib/bargain-order.ts`: the two apps keep their own strings
 * (and their own `strings.ts` key spaces), and a screen may import only from its own app and @dos/ui,
 * so the six lines live once per app rather than in a shared module neither app may reach.
 *
...
export function orderLabel(resolution: OrderResolution, t: Translate): string | null {
  switch (resolution.kind) {
    case 'none':
      return t('o3.rateOrderAny')
    case 'missing':
      return t('o3.rateOrderMissing')
    case 'found':
      return t('o3.rateOrderFound', {
        no: resolution.orderNo ?? t('o3.rateOrderDraft'),
