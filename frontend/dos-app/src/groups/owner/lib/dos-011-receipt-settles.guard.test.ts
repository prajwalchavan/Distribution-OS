 ...
 *
 * The second case READS the MANAGER group's screen (`app/manager/money/index.tsx`) and never imports
 * it: the two panels are one requirement and one guard, and in the one app they are two folders of
 * one project rather than two projects (docs/31 §6.5). A `readFileSync` is not an import, so docs/31
 * §6.4(a) — no group reaches another group — is not contradicted by this file.
 */
...
describe('O11 receipts: what the money settled', () => {
  it('DOS-011: the receipt panel names each bill it settled and states the cash discount', async () => {
    const screen = await read('../../../../app/owner/money/receipts.tsx')
  ...
  it('DOS-033: the manager panel prints the bill number, not its uuid', async () => {
    const screen = await read('../../../../app/manager/money/index.tsx')
