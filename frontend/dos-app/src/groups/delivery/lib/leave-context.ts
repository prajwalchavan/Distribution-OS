 ...
 * answered — on a browser that keeps nothing, the queued deliveries went with it. `Chrome` in `app/_layout.tsx` hands
 * down its own `leave`, the one the account menu calls, so every sign-out button takes the same flow (`leave.test.ts`
 * reads every screen to keep it so).
 */
