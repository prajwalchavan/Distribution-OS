...
import { instantWithClock, shiftDays, shortDate, today } from '../../src/groups/warehouse/lib/dates'
import {
  Async,
  Panel,
  PageTabs,
...
export default function Home(): React.JSX.Element {
  ...
  return (
    <Screen title={t('w1.title')} context={session?.tenant.displayName}>
      <Stack gap={6}>
        <PageTabs group={go.href('/')} active={go.href('/')} />
            ...
            <Group>
              {(grns.data?.items ?? []).map((grn) => (
                <ListRow
                  ...
                  onPress={() => {
                    router.push(go.href(`/inbound/${grn.id}`))
                  }}
                />
              ))}
            </Group>
            ...
            <Group>
              {(packs.data?.items ?? []).map((pack) => (
                <ListRow
                  ...
                  onPress={() => {
                    router.push(go.href(`/pack/${pack.orderId}`))
                  }}
                />
              ))}
            </Group>
            ...
            <Group>
              {(counts.data?.items ?? []).slice(0, 5).map((count) => (
                <ListRow
                  ...
                  onPress={() => {
                    router.push(go.href(`/stock/counts/${count.id}`))
                  }}
                />
              ))}
            </Group>
