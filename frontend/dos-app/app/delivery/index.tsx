...
import { FillingNote, LocalAsync, Panel, StopChip, pl } from '../../src/groups/delivery/lib/ui'
          ...
          <Stack gap={3}>
            {next === null ? null : (
              <Button
                ...
                fullWidth
                onPress={() => {
                  router.push(go.href(`/stop/${next.id}`))
                }}
              />
            )}
            <Button
        ...
        <Row gap={8} wrap>
          {bool(trip.van_sales_enabled) && road ? (
            <Button
              ...
              variant="secondary"
              onPress={() => {
                router.push(go.href(next === null ? '/' : `/stop/${next.id}/van-sale`))
              }}
