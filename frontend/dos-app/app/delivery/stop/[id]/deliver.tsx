...
import {
  ...
  recordOrSave,
  type DoorstepGate,
} from '../../../../src/groups/delivery/lib/doorstep'
import {
  captureProof,
  storeProof,
  MAX_INLINE_BASE64,
  type CapturedProof,
  type StoredProof,
} from '../../../../src/groups/delivery/lib/proof'
import { useQueueDelivery, type QueuedPod } from '../../../../src/groups/delivery/lib/queue'
import { LocalAsync, Panel, Field } from '../../../../src/groups/delivery/lib/ui'
    ...
    void (async () => {
      try {
        ...
        if (result.via === 'phone') {
          ...
          router.replace(go.href(doorDoneHref(stopId ?? '', { code: 'kept' })))
        }
      } catch (thrown) {
        haptics.error()
