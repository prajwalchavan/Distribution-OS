...
import { shortDate, shortInstant } from '../../../src/groups/manager/lib/dates'
import {
  loadOutHistory,
  loadOutQueue,
  nextChallanPoll,
} from '../../../src/groups/manager/lib/load-out'
    ...
    const attempt = (tries: number): void => {
      void api.api.warehouse.challans.pdf({ id: challanId }).then(
        (result) => {
          ...
          const readyUrl = absoluteUrl('manager', result.url)
    ...
    <Screen
      ...
      chips={
        <PageTabs
          ...
          active={routeFor('manager', '/fulfilment/load-out')}
        />
      }
      actions={
        <Segments
                ...
                <Stack gap={2}>
                  <Button
                    ...
                    onPress={() => {
                      ...
                      const printUrl = absoluteUrl('manager', challanUrl)
                      if (printUrl !== null) {
