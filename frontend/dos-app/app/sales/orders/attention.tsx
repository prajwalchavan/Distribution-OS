...
import { useLocalState, useOrder, useShop } from '../../../src/groups/sales/lib/local'
import { PageTabs, Panel } from '../../../src/groups/sales/lib/ui'
...
export default function NeedsAttention(): React.JSX.Element {
  ...
  return (
    ...
    >
      <Stack gap={5}>
        <PageTabs group={go.href('/orders')} active={go.href('/orders/attention')} />
                  ...
                  <Row gap={2} padX={3} padY={2} wrap>
                    ...
                    {item.error.table === 'sales_orders' ? (
                      <Button
                        ...
                        variant="ghost"
                        onPress={() => {
                          go.push(`/orders/${item.error.rowId}`)
                        }}
                      />
                    ) : null}
                  </Row>
