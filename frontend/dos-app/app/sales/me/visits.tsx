...
import { instantWithClock } from '../../../src/groups/sales/lib/dates'
import { useAllShops, useLocalState, useMyVisits } from '../../../src/groups/sales/lib/local'
import { LocalAsync, PageTabs, useMyUserId } from '../../../src/groups/sales/lib/ui'
...
export default function Visits(): React.JSX.Element {
  ...
  return (
    ...
    >
      <Stack gap={4}>
        <PageTabs group={go.href('/me')} active={go.href('/me/visits')} />
          ...
          <Group>
            {rows.map((visit) => (
              <ListRow
                ...
                onPress={() => {
                  go.push(`/shops/${visit.retailer_id}`)
                }}
              />
            ))}
          </Group>
