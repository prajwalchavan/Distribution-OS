...
import { Async, PageTabs, Panel } from '../../../src/groups/sales/lib/ui'
import { formatBps, useWord } from '../../../src/groups/sales/lib/words'
...
export default function Drafts(): React.JSX.Element {
  ...
  return (
    ...
    >
      <Stack gap={4}>
        <PageTabs group={go.href('/me')} active={go.href('/me/drafts')} />
...
function DraftSheet({
  ...
  const confirm = useMutation(
      ...
      }),
    {
      ...
      onSuccess: (result) => {
        onClose()
        go.push(`/orders/${result.order.id}`)
      },
    },
  )
