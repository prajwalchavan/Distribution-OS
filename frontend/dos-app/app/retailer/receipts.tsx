...
import { instantWithClock } from '../../src/groups/retailer/lib/dates'
import { useMyShop } from '../../src/groups/retailer/lib/shop'
import { Async, Field, PageTabs, Panel } from '../../src/groups/retailer/lib/ui'
...
export default function Receipts(): React.JSX.Element {
  ...
  const url = absoluteUrl('retailer', document.data?.url)

  return (
    <Screen title={t('r13.title')} context={distributor} testID="r13-screen">
      <Stack gap={5}>
        <PageTabs group={go.href('/dues')} active={go.href('/receipts')} />
        <Async
          state={[my, receipts]}
          rows={5}
          empty={rows.length === 0}
