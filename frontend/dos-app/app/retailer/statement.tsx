...
import { Money, Row, Screen, Segments, Stack, Txt, useColors, useGo, useStrings } from '@dos/ui'
import { useState } from 'react'

import { longDate, rangeOf, type RangeId } from '../../src/groups/retailer/lib/dates'
import { useMyShop } from '../../src/groups/retailer/lib/shop'
import { Async, PageTabs, Panel } from '../../src/groups/retailer/lib/ui'
...
export default function Statement(): React.JSX.Element {
  ...
  return (
    <Screen title={t('r6.title')} context={session?.tenant.displayName} testID="r6-screen">
      <Stack gap={5}>
        <PageTabs group={go.href('/dues')} active={go.href('/statement')} />
        <Segments
          testID="r6-range"
          items={[
