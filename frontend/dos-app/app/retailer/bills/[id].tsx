...
import { instantWithClock, longDate } from '../../../src/groups/retailer/lib/dates'
import { addressLine } from '../../../src/groups/retailer/lib/shop'
import { Async, Field, Panel, billFamily } from '../../../src/groups/retailer/lib/ui'
import { useCreditNoteWord, useWord } from '../../../src/groups/retailer/lib/words'
...
export default function BillDetail(): React.JSX.Element {
  ...
  const url = absoluteUrl('retailer', pdf.data?.url)
            ...
            {bill.amountDuePaise > 0 ? (
              <Button
                ...
                onPress={() => {
                  ...
                  go.push(billId === null ? '/pay' : `/pay?bill=${billId}`)
                }}
                testID="r4-pay"
              />
            ) : null}
                ...
                <Async state={[deliveries, delivery]} rows={2}>
                  {delivery.data === undefined ? (
                    ...
                    <Stack gap={3}>
                      ...
                      {proofs
                        .map((proof) => ({ proof, url: absoluteUrl('retailer', proof.readUrl) }))
              ...
              <Panel title={t('r4.problem')} testID="r4-problem">
                <Stack gap={3}>
                  ...
                  <Button
                    ...
                    variant="secondary"
                    onPress={() => {
                      go.push(`/returns?invoiceId=${billId ?? ''}`)
                    }}
                    testID="r4-report"
                  />
                </Stack>
