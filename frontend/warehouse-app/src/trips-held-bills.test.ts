...
async function readScreen(): Promise<string> {
  ...
  return readFileSync(fileURLToPath(new URL('../app/load/trips.tsx', import.meta.url)), 'utf8')
}
...
describe('W10 Trips: bills held on the road', () => {
  it('DOS-172: W10 lists the planning board\'s held bills under the plannable bills, greyed and never pressable, each saying "Out on <trip> — back after check-in"', async () => {
