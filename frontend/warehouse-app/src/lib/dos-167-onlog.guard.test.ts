/**
 * DOS-167 ruling 3 (dd), S-139 — the warehouse app's offline lines have somewhere to go.
 *
...
async function readLayout(): Promise<string> {
  ...
  return readFileSync(fileURLToPath(new URL('../../app/_layout.tsx', import.meta.url)), 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/^\s*\/\/.*$/gm, '')
}

describe('DOS-167 the warehouse app writes the offline lines somewhere they can be read', () => {
  it('DOS-167 the warehouse app passes onLog to OfflineProvider', async () => {
