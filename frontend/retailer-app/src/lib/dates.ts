 ...
 * own window maths does, so the window the app asks for is the window the register serves.
 */
...
/** The ranges the statement offers, as `{ from, to }` IST dates. */
...
/** An ISO instant → `4:45 pm` IST, with no date. A delivery ETA has no room for one. */
