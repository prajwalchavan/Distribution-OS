...
async function readScreen(): Promise<string> {
  ...
  return readFileSync(fileURLToPath(new URL('../app/settings/index.tsx', import.meta.url)), 'utf8')
}
...
function elementWithTestId(code: string, id: string): { tag: string; source: string } {
  ...
  expect(at, `no element carries ${prop} in app/settings/index.tsx`).toBeGreaterThan(-1)
