 ...
 * Approvals screen already reads, and the server's own rule is written against those rows — an order confirms only when none of them is pending (`approvals.service.ts:195`) — so the
 * column states them.
 *
 * Pure on purpose: no query, no translation, no React. The screen passes the pending rows and turns the
 * kinds into words with `useWord()`.
 */
