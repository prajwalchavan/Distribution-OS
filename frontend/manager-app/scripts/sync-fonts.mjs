...
/**
 * Where the design system keeps the binaries.
 *
 * `require.resolve('@dos/ui/package.json')` is tried first but usually FAILS: the kit's `exports`
 * map has no `./package.json` entry, and Node will not resolve a subpath a package does not publish.
 * So the real answer is the walk below — from this app up to the workspace root, looking for
 * `libs/ui/assets/fonts`. It has to be a walk rather than a fixed `../../`, because an app sits one
 * directory shallower than the template it was generated from and a fixed path is right for exactly
 * one of them (which is how this app first shipped with four `OTS parsing error` warnings per page).
 */
function kitFontsDir() {
  ...
  for (let up = 0; up < 5; up++) {
    const candidate = resolve(dir, 'libs/ui/assets/fonts')
