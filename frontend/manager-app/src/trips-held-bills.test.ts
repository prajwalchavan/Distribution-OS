/**
 * DOS-172 — M7 Trips: the planning board's held bills, shown and never offered.
 *
...
function between(code: string, from: string, to: string): string {
  ...
  expect(start, `app/fulfilment/trips.tsx has no ${from}`).toBeGreaterThan(-1)
  const end = code.indexOf(to, start)
  expect(end, `app/fulfilment/trips.tsx has no ${to} after ${from}`).toBeGreaterThan(start)
...
function elementWithTestIdPrefix(code: string, prefix: string): { tag: string; source: string } {
  ...
  expect(at, `no element carries ${prop}…}\` in app/fulfilment/trips.tsx`).toBeGreaterThan(-1)
...
describe('M7 Trips: bills held on the road', () => {
  it('DOS-172: M7 lists the planning board\'s held bills under the plannable bills, disabled and never pressable, each saying "Out on <trip> — back after check-in"', async () => {
    ...
    const billAt = list.indexOf('testID={`trip-bill-${')
    const heldAt = list.indexOf('testID={`trip-held-${')
    ...
    expect(catalogue['m7t.heldOnTrip']).toBe('Out on {trip} — back after check-in')
  })

  it('DOS-172: an M7 board page that carries only held bills draws them instead of "Every packed bill is already on a trip"', async () => {
    const code = withoutComments(await readScreen())
    const list = between(code, 'const billList =', 'const crewRow =')
