/**
 * DOS-167 — the words of the sheet a driver sees when signing out, or switching distributor, while this
 * phone still holds deliveries, receipts or returns the office has not got.
 *
...
export interface LeaveSteps {
  /**
   * What waits in this driver's file, counted once the engine has opened it — never the status snapshot,
  ...
  /** This driver's files at their other distributors: deleted where nothing waits in them. */
 ...
 * delete a delivery this driver had kept on the phone — with no sheet. A file that cannot be counted at
 * all is never deleted: the driver is signed out keeping whatever it holds.
 */
 ...
 * to the sign-in form, and "Send now" has already sent with the live session before this runs. `keepQueue: false` deletes this driver's file unless the engine's own count
...
export async function leaveNow(to: Leaving, keepQueue: boolean, steps: LeaveSteps): Promise<void> {
  ...
  try {
    ...
    // Best effort, file by file: a file left behind is still under this driver's own name.
  }
