...
/**
 * The id of THIS install, for `@dos/offline` and for `/gps/points`.
 *
 * Deliberately the same key `boot()` writes and `TokenStorage.getDeviceId()` returns: docs/27 §4 wants
 * the sync device and the `auth_sessions` device to be one device, and ADR 0012 dedupes GPS on
 * `UNIQUE(tenant, trip, device, recordedAt)` — so the phone that recorded a delivery, the phone that
 * queued the receipt and the phone that dropped the breadcrumb have to be the same phone or none of
 * the three can be traced to it. Reading it needs no await because `boot()` has already primed the
 * synchronous cache, and the root layout does not mount anything that asks until it has.
 */
