...
ALTER TABLE "external_party_codes" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
...
ALTER TABLE "retailer_purchase_history" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
...
ALTER TABLE "tenant_brands" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
...
--> statement-breakpoint
-- 7. Cross-tenant jobs (outbox relay, retention sweeps, rollups) run as app_worker, which bypasses RLS.
--    It is NOT a login role for the API; only the worker's cross-tenant paths SET LOCAL ROLE app_worker.
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_worker') THEN CREATE ROLE "app_worker" BYPASSRLS; END IF;
END $$;--> statement-breakpoint
GRANT USAGE ON SCHEMA public TO app_worker;--> statement-breakpoint
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO app_worker;--> statement-breakpoint
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO app_worker;--> statement-breakpoint
