...
### DOS-167: the judge says NOT CLOSED — "goes first" fails on the web (2026-09-19, night)

Run `wf_dae59dd1-114` finished: 16 agents, the five amendments merged as `609388b`, then six walks and Fable's judgement.

**What is proven.** Two of the founder's three clauses hold on every target, and the web store is now honest. Unsent changes survive sign-out and never reach anybody else — on **Android** (sales, delivery and warehouse, with real ops queued under a three-way network cut and isolation proven against a fully synced other person, the office row landing exactly once), on **iOS**, and on **web** (tab 1's OPFS file — header, size, magic and queue — untouched by a second tab, by a corrupted file, or by a 15-second abandoned open; the memory fallback at 752 ms with a second engine opening over it; the late handle closed and never destroyed, and the queued order recovered and sent). **"Goes first" holds on Android and iOS to the millisecond.**

**What fails, and why the P0 stays open.** On the web, "goes first" does not hold. Measured twice, one tab, no harness trick: a plain sign-in over a file holding an unsent order **pulled first and uploaded only at +60 753 ms**, on the poll tick; and a page that had booted offline **made no call for 49.5 s** after a genuine online event, then pulled before uploading. Cause, read only after observing it: `engine.ts:452-488` `start()` runs `sync('start')` and never `flush()`, and the one pre-pull flush — `applyManifest`'s stale branch — is reached only when `dropReadSet` has nulled the manifest state, so a sign-out from another tab, a crash or a closed window leaves the promise unmet. A second gap on the same clause: `setNetworkHint(true)` returns early because `radio()` falls back to `navigator.onLine`, already true inside the online handler, so a page that booted offline never gets its reconnect flush. Filed as **DOS-183 (P1, on the P0 path)**. Nothing is lost — the order is sent — but the founder chose that clause deliberately.

**Two P1s the walks found that have nothing to do with DOS-167:**
- **DOS-181** — on Android the delivery app's footer button ("Record the delivery" / "Save on this phone") **never fires `onPress`** (`deliver.tsx:283`, `:482`). The pressed style renders, so the view takes the tap, and nothing is saved: no row, no outbox entry, no log line. **A driver cannot close a stop at the door.** Two earlier provers hit this and both read it as their own tooling failing; it is not.
- **DOS-182** — the warehouse pick sheet, opened with no signal, hides Picked and Short for **5 min 48 s** (measured) because `locked = liveStatus === undefined || notStarted` waits for a read to reach the office, while the wave is already on the phone.

  **Both mechanisms corrected when the fixes were merged (merge review, 2026-09-20). The two observations stand; the causes written above do not.** DOS-181's press was never dead — it was ACCEPTED and refused off-screen, at `commit()`'s `podRequired && proof === null` on a credit shop with no photograph, leaving one sentence in `d4-error` a screen and a half below the thumb with no haptic (the same run's `d5-record` took its press, and-run.log:625). DOS-182's phone HELD the sheet row as `picking` (wh-run.log:245), so nothing was locked: two standing sentences above the list pushed the first card's Picked / Short under the sticky bar, and the prover reads only the first viewport and never scrolls. Full corrections in `QA/findings/12-batch2-new-findings.md` under each finding.
...
### DOS-183, DOS-181 and DOS-182 fixed and merged; DOS-167 is one walk from closed (2026-09-20)

Run `wf_1f0628d4-535`, 18 agents. **Merged: `f23a9a7`** (DOS-183, the offline engine) and **`71fa9c3`** (DOS-181, DOS-182, the two app defects).

**DOS-183 is proven on the browser, in milliseconds, twice on two independent profiles.** A sign-in over a kept file now uploads at **+287 ms**, before the manifest at +405 and the pull at +417; a page that booted with no network uploads **+38 ms** after the browser's online event, where it used to wait 49.5 s. Every op is accepted exactly once (`sync_ops` one row per op, the raw network log re-read: 401 → refresh 200 → 200), each order reaches the office once, and the file survives a cross-tab sign-out and a closed window. Fable's design is worth keeping as the pattern: one private `drain(reason)` = `flush({ pullAfter: false })` then `sync(reason)`, called from `start()`, from `setNetworkHint(true)` and from the poll tick; `setNetworkHint` now decides on what the engine itself knew rather than `navigator.onLine`, so a reconnect can never be swallowed; and a flush that fails links as `flushChain.then(run, run)` so the caller still sees the rejection while the chain never carries it forward — the pull follows regardless. docs/27 §4, §5, §10, §12, §13 and §14 changed with it.

**DOS-181 — the dead delivery button — was proven by hand on the device**, both presses, online and offline. Worth recording how nearly it was missed: the walk's brief named the engine-lane commit as the base, and the prover noticed that commit does **not** contain the DOS-181 fix and walked the right one instead, saying so. A less careful agent would have walked the old screen and reported a pass.
