...
if [ -n "$TITLE" ] && ! ui texts | grep -q "Distribution OS - $TITLE"; then adb exec-out screencap -p > "$OUT/$TAG-9-wrongbundle.png"; echo "BUNDLE_MISMATCH $TAG expected '$TITLE': $(ui texts)"; exit 5; fi
