...
**Owed by the founder for day 7 (go-live):** `distributionos.in` is BOUGHT (Hostinger, 2026-09-21) and its nameservers are being pointed at Cloudflare (`kay` / `lloyd.ns.cloudflare.com`); still owed: the Cloudflare API token (Edit zone DNS + Pages Edit); the Oracle account (Mumbai — irreversible; a real credit card, PIN-debit/prepaid/virtual refused) + its API key config; gated-or-public (recommended gated); the data extract if it is coming.
