Stage: 1
Current phase: 2 — Cross-role end-to-end business flow: RUN (holds with findings, `QA/09-cross-role-workflows.md`); its six P1s are being fixed before the simulation. Batch 2 (Phase 4) is merged: 153 of 158.
...
## Running right now — DAY 1 → 2 of seven (Sun 21 Sep, evening)

| Run | Task | What it is |
|---|---|---|
| ~~`day1-chain.js`~~ | done | **Phase 2 RUN — `holds-with-findings`** (`QA/09-cross-role-workflows.md`, 476 evidence files in `QA/evidence/chain/`). The happy chain held end to end with the numbers read out of SQL at every hop: SO-0879 → credit approval (the limit stayed ₹50,000) → PICK-0079 (FEFO exact, 19 lots) → INV/9007 issued at pack with the tenant's own logo → TRIP-0001, load sheet approved from the manager app, challan DC-0083 → delivered, ₹6,000 cash + UPI → settled, banked → the owner's Today equals SQL; the console saw counts and no rupee. **The days that go wrong found what Phase 2 exists to find — 20 new findings, DOS-185..204 + S-170..174, six of them P1:** DOS-195 pack relieves the godown with no counterpart on the vehicle, so undelivered and refused goods VANISH from inventory (the simulation's arithmetic would fail on any failed delivery); DOS-185 a free-goods scheme is priced into the order and then never picked, billed or delivered; DOS-191 a rejected order shows the rep a raw enum and loses the manager's note; DOS-196 no desk screen shows what came back undelivered; DOS-197 a failed or refused delivery tells the shop nothing and leaves the bill payable and ageing; DOS-186 the owner's stock ledger cannot reach today. The blind verifier: 47 claims, 40 TRUE, 1 FALSE (the `sellable_stock` view carries no location predicate — DOS-204), 6 not decidable from the database. |
| `phase2-p1-fixes` | `wo7optdp2` | **The six P1s and DOS-204, before the simulation** — three lanes: stock (DOS-195 vehicle ledger, DOS-204 the view, DOS-186), free goods through the chain (DOS-185), the undelivered flow (DOS-196, DOS-197, DOS-191, + DOS-203). Test-first, adversarial verify, integrate; the architect merges. **The simulation does NOT start until these are on main** — it would only rediscover them on its ugly day. |
...
| ~~`role-election.js`~~ | done | **MERGED `acd2bc0`** — docs/29 §2 with ruling B1 (a refresh re-elects; a withdrawn grant ends the session). Verify pass; review's blocker (a manager and an accountant's extras) fixed on the branch; integ-verify pass; the architect read the branch and merged by hand. Migration 0057 (two columns, no policy/grant/role); PERMISSIONS 391 → 392; every service role list unchanged; 1 650 matrix tests + rls 95 + core 809 green. Owed: the browser walk of an elected token (the one-app gate covers it); the as-built row is in docs/22. |
| `one-app-merge.js` | see /workflows | **docs/29 §3 under the docs/31 ruling — LAUNCHED** the moment election landed: Ground (precondition check, §1.3/§1.4 re-derived from HEAD, isActive fix, routeFor, GROUP_OF, the dos-app skeleton) → six move lanes → assemble (root layout, the Continue-as chooser, strings swap, serviceFor / apiUrlFor / actAs) → retire (guards, the six apps deleted, launch.json, generate-readmes, docs) → gate → blind verifier → the architect merges. |
...
| `walk-findings-fix` | see /workflows | S-176 + S-177 (one lane, backend, stops for the architect — tax and a sync-pull column list are money and a permission surface), the D8 display cluster (one lane, delivery app, walked in a browser), and the S-number collision repair in the findings file — **DONE 2026-09-21**: the smoke-failures block (08:17) and the money-web block (09:28) both took S-166/S-167; the later-filed money-web rows are now **S-183** (`d8.uncounted` never renders offline) and **S-184** (the van-sale disabled sentence), its S-168/S-169 unchanged, and the smoke block's four rows are registered as S-164..S-167. The file now runs S-01..S-184 with no duplicate and no gap. |
