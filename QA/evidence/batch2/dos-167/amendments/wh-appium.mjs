...
export const LOG = process.env.WH_LOG || EV + '/wh-run.log'
...
export async function newSession(opts = {}) {
  const caps = {
    alwaysMatch: {
      ...
      'appium:appPackage': 'in.distributionos.warehouse',
...
export function sqliteFiles() {
  try {
    return adb(['shell', 'run-as', 'in.distributionos.warehouse', 'ls', '-la', 'files/SQLite']).trim()
  } catch (e) {
...
export function pullDb(dbName, tag) {
  ...
  adb(['shell', `run-as in.distributionos.warehouse cat files/SQLite/${dbName} > ${tmp}`])
