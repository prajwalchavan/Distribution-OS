...
describe('retailer-service', () => {
  ...
  it('answers /health and publishes its OpenAPI document with only its own routes', async () => {
    ...
    expect(spec.info.title).toContain('Retailer service')
  ...
  it('documents bearer auth: securityScheme, x-roles per operation, no security on public routes', async () => {
    ...
    expect(me?.['x-roles']).toContain('retailer')
  ...
  it('refuses roles this service does not serve, before any business logic', async () => {
    ...
    const denied = await app.inject({
      method: 'GET',
      url: '/tenancy/me',
      headers: await bearer({ tenantId, actorId, role: 'salesperson' }),
    })
    expect(denied.statusCode).toBe(403)
    expect(denied.json<{ message: string }>().message).toBe(
      'retailer-service does not serve the salesperson role',
    )
    ...
    // The gate let the retailer through; with no membership for the fake ids the handler itself answers 401 (oRPC).
