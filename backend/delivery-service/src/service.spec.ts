...
describe('delivery-service', () => {
  ...
  it('answers /health and publishes its OpenAPI document with only its own routes', async () => {
    ...
    expect(spec.info.title).toContain('Delivery service')
  ...
  it('documents bearer auth: securityScheme, x-roles per operation, no security on public routes', async () => {
    ...
    expect(me?.['x-roles']).toContain('delivery')
  ...
  it('refuses roles this service does not serve, before any business logic', async () => {
    ...
    expect(denied.json<{ message: string }>().message).toBe(
      'delivery-service does not serve the retailer role',
    )

    const allowed = await app.inject({
      method: 'GET',
      url: '/tenancy/me',
      headers: await bearer({ tenantId, actorId, role: 'delivery' }),
    })
    // The gate let the delivery through; with no membership for the fake ids the handler itself answers 401 (oRPC).
