 ...
 *  - sync_tombstones: 180 days (`SYNC_TOMBSTONE_RETENTION_DAYS`; a device that has been away longer
 *    re-snapshots instead of catching up on deletions — that is the second job of the manifest's
 *    `schemaVersion`). `app_worker` is granted DELETE on it by migration 0040 and `app_rw` is not:
 *    the triggers are the only author, and this sweep the only reaper.
...
export async function runRetention(db: Db): Promise<Record<string, number>> {
  const sweeps: Record<string, ReturnType<typeof sql>> = {
    ...
    sync_tombstones: sql`DELETE FROM sync_tombstones WHERE ctid IN (SELECT ctid FROM sync_tombstones WHERE deleted_at < now() - interval '180 days' LIMIT 5000)`,
