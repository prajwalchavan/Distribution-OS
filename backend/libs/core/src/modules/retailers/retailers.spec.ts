...
describeDb('retailers (DATABASE_URL)', () => {
  ...
  it('takes the code past shops the series row never counted, instead of colliding', async () => {
    // A tenant can hold codes the RET/ALL counter knows nothing about: seeded or imported shops, or codes spent
    // by an older series row keyed with the financial year. Rewinding the counter reproduces exactly that.
    await db.execute(
      sql`update numbering_series set next_no = 1
          where tenant_id = ${tenantId} and series_code = 'RET' and fy = 'ALL'`,
    )
    const third = await call<{ item: RetailerRow }>(app, owner, 'POST', '/retailers', {
      idempotencyKey: `ret3-${run}`,
      id: uuidv7(),
      name: 'Behind-the-counter Traders',
      phone: `+919${run}6`,
      stateCode: '27',
    })
    expect(third.status).toBe(200) // never 500: R-0001 and R-0002 are already spent
    expect(third.body.item.code).toBe('R-0003')

    const fourth = await call<{ item: RetailerRow }>(app, owner, 'POST', '/retailers', {
      idempotencyKey: `ret4-${run}`,
      id: uuidv7(),
      name: 'Fourth Traders',
      phone: `+919${run}7`,
      stateCode: '27',
    })
    expect(fourth.body.item.code).toBe('R-0004') // the counter stays healed
  })
