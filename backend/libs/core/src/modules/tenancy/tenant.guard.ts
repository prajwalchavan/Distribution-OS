...
export interface AccessClaims {
  ...
  tid: string | null
  /** Membership role in `tid`; null exactly when `tid` is null. */
 ...
 *   4. 'authenticated' needs nothing more (tenant context is entered when the token carries one);
 *   5. a role list needs an active tenant + role, then this SERVICE must serve the role (403), then
...
export class TenantGuard implements CanActivate, OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    this.verifier = createTokenVerifier(await loadAuthKeys())
  }
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!this.verifier) throw new ServiceUnavailableException('auth keys not loaded yet')
  ...
  private requireServed(role: MembershipRole): void {
...
function enterTenant(claims: AccessClaims, tenantId: string, role: MembershipRole): void {
...
/** The only part of the request the guard reads; keeps it independent of the HTTP adapter's types. */
interface RequestWithHeaders {
  headers: Record<string, string | string[] | undefined>
}

function bearerToken(req: RequestWithHeaders): string | null {
