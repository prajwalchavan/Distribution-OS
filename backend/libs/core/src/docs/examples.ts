...
const NOTES: Record<string, (ctx: ExampleContext) => string | undefined> = {
  ...
  'delivery.trips.depart': () =>
    'planned → loading → active: press start-loading first. Needs the driver to have answered the location notice (POST /delivery/consents). Refused 409 load_sheet_not_confirmed while a load sheet of the trip is still a draft, and 409 bill_not_loaded while a bill on the trip has not been counted out on a confirmed load sheet (the load-out dispatches it; departing dispatches nothing); the godown (warehouse role) may not call it.',
