...
/**
 * One group of the ONE APP (docs/31): a visible route base, the service it talks to, and its screens.
 *
 * The six per-role apps were retired at the merge and this is what replaced their six README entries.
 * A group is not a service — the server still runs one service per role (docs/29 §0) — so the README
 * lists six endpoint tables rather than pretending there is one API.
 */
export interface AppGroupReadme {
  /** The heading this group gets under Screens: "Owner", "Sales", "Shop". */
  readonly heading: string
  /** The visible route base every screen of this group sits under: `/owner`, `/sales` (ruling Q1). */
...
/**
 * The README of the ONE APP — one Expo codebase, six route groups, six services.
 *
 * `renderAppReadme` below serves an app that talks to exactly ONE service, which after the merge is
 * the platform console and nothing else. This one serves the app that talks to six: the "Talks to"
 * line becomes a table of group → base → service → port, the screens are listed under a heading per
 * group, and the endpoint table is repeated once per service because the Roles column and the paths
 * differ between them. Everything else — the generated banner, the Sign in paragraph, the Run block —
 * is word for word what a single-service app gets, so the two READMEs read as one family.
 */
export function renderOneAppReadme(opts: {
  name: string
  title: string
  blurb: string
  run: string
  env: string
  groups: readonly AppGroupReadme[]
}): string {
  const services = [
    '| Group | Routes | Service | Port |',
    '|---|---|---|---|',
    ...opts.groups.map(
      (group) =>
        `| ${group.heading} | \`${group.base}/…\` | ${group.service.title} | ${group.service.defaultPort} |`,
    ),
  ].join('\n')

  const screens = opts.groups
    .map(
      (group) =>
        `### ${group.heading} (\`${group.base}\`)\n\n${group.screens.map((screen) => `- ${screen}`).join('\n')}`,
    )
    .join('\n\n')

  const endpoints = opts.groups
    .map((group) => {
      const procs = collectProcedures(pickContract(group.service.contractKeys))
      const table = [
        '| Method | Path | Used for | Roles |',
        '|---|---|---|---|',
        ...procs.map(
          (p) =>
            `| ${p.method} | \`${p.path}\` | ${p.summary} | ${describePermission(p.permission)} |`,
        ),
      ].join('\n')
      return `### ${group.heading} → ${group.service.title} (:${group.service.defaultPort})\n\nFull request/response samples are in \`backend/${group.service.name}-service/README.md\`.\n\n${table}`
    })
    .join('\n\n')

  return `<!-- GENERATED by \`pnpm docs:readme\` (endpoint tables) — edit the prose in backend/libs/core/src/docs/readme.ts or the app entry in backend/tools/generate-readmes.mts -->

# ${opts.title} (\`${opts.name}\`)

${opts.blurb}

## Groups and the services behind them

${services}

The role in the access token is the ELECTED role (docs/29 §2): it decides which group the root layout
opens, and which service each call leaves for. A person whose membership permits more than one role
chooses at **Continue as …** after the password, and changing role mints a new token.
...
/**
 * README for a frontend app that talks to ONE service: which service, and the endpoints it uses.
 * The one app, which talks to six, is `renderOneAppReadme` above.
 */
