...
const ROLE_ABBR: Record<MembershipRole, string> = {
  owner: 'O',
  manager: 'M',
  accountant: 'A',
  salesperson: 'S',
...
export function renderServiceReadme(def: ServiceDefinition): string {
  ...
  const matrixHeader = `| Procedure | ${ALL_ROLES.map((r) => ROLE_ABBR[r]).join(' | ')} |`
  const matrixSep = `|---|${ALL_ROLES.map(() => '---').join('|')}|`
  const matrixRows = procs.map((p) => {
    const cells = ALL_ROLES.map((r) => (reachableOnThisService(p, def, r) ? '✓' : '–'))
