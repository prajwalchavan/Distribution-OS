...
export const enteredUnit = pgEnum('entered_unit', ['piece', 'inner', 'case'])
...
export const salesOrders = pgTable(
  ...
  (t) => [
    ...
    // A retailer edits its own draft and cancels its own submitted order (docs/18: the USING clause used
    // to allow 'draft' only, so the cancel path had to escalate to the system role).
    pgPolicy('sales_orders_retailer_update', {
      ...
      using: sql`tenant_id = (SELECT current_setting('app.tenant_id', true)) AND (SELECT current_setting('app.actor_role', true)) = 'retailer'
        AND created_by = (SELECT current_setting('app.actor_id', true)) AND state IN ('draft', 'submitted')`,
...
export const salesOrderLines = pgTable(
  'sales_order_lines',
  {
    ...
    /** As entered, in the unit the rep/retailer thinks in, with the pack size that applied (docs/17 A3). */
    enteredQty: integer('entered_qty').notNull(),
    enteredUnit: enteredUnit('entered_unit').notNull().default('piece'),
    packSizeAtEntry: integer('pack_size_at_entry').notNull().default(1),
  ...
  (t) => [
    /** Delta pull for the offline device: rows changed since its cursor (own sync, docs/22 §8). */
    index('sales_order_lines_updated_idx').on(t.tenantId, t.updatedAt),
...
export const orderStateTransitions = pgTable(
  ...
  (t) => [
    ...
    // A retailer records transitions for its own orders (submit, cancel) without the service having to
    // escalate to the system role (docs/18). Append only: no retailer update/delete policy, and the
    // append-only trigger from migration 0003 refuses both anyway.
    pgPolicy('order_state_transitions_retailer_insert', {
      for: 'insert',
      to: appRw,
      withCheck: sql`tenant_id = (SELECT current_setting('app.tenant_id', true)) AND (SELECT current_setting('app.actor_role', true)) = 'retailer'
