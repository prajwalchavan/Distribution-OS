-- Hand-written companion to 0055_inbound_reports_expand.sql (DOS-103): a shop can report a problem or ask for a
-- return from the app, and it lands in the office's inbound queue with the bill or delivery it is about.
--
-- WHAT 0055 DID. `inbound_messages` gains `kind`, `ref_type`, `ref_id` and `created_by`, all nullable (a text the
...
BEGIN
  ...
  IF forced IS DISTINCT FROM true THEN
    RAISE EXCEPTION '0056: inbound_messages must keep FORCE ROW LEVEL SECURITY';
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policy p WHERE p.polrelid = 'public.inbound_messages'::regclass AND p.polcmd = '*'
  ) THEN
    RAISE EXCEPTION '0056: inbound_messages must not carry a FOR ALL policy';
  END IF;
  ...
  IF cmd IS DISTINCT FROM 'INSERT' OR check_expr IS NULL THEN
    RAISE EXCEPTION '0056: inbound_messages_shop_insert must exist as an INSERT policy with a WITH CHECK; found %',
      coalesce(cmd, 'no such policy') USING ERRCODE = 'undefined_object';
  END IF;
  IF position('in_app' IN check_expr) = 0 THEN
    RAISE EXCEPTION '0056: a shop may insert an in_app report only, never a whatsapp or sms row; found %', check_expr
      USING ERRCODE = 'insufficient_privilege';
  END IF;
  IF position('created_by' IN check_expr) = 0 OR position('retailer_links' IN check_expr) = 0 THEN
    RAISE EXCEPTION '0056: a shop report must be signed with the caller''s own id and name its own linked shop; found %',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0056: triage stays the desk''s — no shop UPDATE or DELETE policy on inbound_messages'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
END;
$$;
