-- Hand-written companion to 0048_grns_supplier_expand.sql (QA DOS-050): the gate's receipt row names the
...
BEGIN
  ...
  IF forced IS DISTINCT FROM true THEN
    RAISE EXCEPTION '0049: grns must keep FORCE ROW LEVEL SECURITY';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policy p WHERE p.polrelid = 'public.grns'::regclass AND p.polcmd = '*') THEN
    RAISE EXCEPTION '0049: grns must not carry a FOR ALL policy';
  END IF;
  FOR nullable IN
    SELECT column_name FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'grns'
       ...
       AND is_nullable = 'NO'
  LOOP
    RAISE EXCEPTION '0049: grns.% must stay nullable; a receipt booked before this migration names no supplier', nullable;
  END LOOP;
  IF (SELECT count(*) FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'grns'
         AND column_name IN ('supplier_id', 'supplier_invoice_no')) <> 2 THEN
    RAISE EXCEPTION '0049: grns must carry both supplier_id and supplier_invoice_no';
  END IF;
  ...
  IF stragglers > 0 THEN
    RAISE EXCEPTION '0049: % goods receipts still name no supplier after the backfill', stragglers;
  END IF;
END;
$$;
