...
export class TenantCatalogController {
  ...
  upsertListing(@OwnsReply() _reply: unknown) {
    return implement(contract.tenantCatalog.upsertListing).handler(({ input }) => this.svc.upsertListing(input))
  }
  ...
  upsertSupplier(@OwnsReply() _reply: unknown) {
    return implement(contract.tenantCatalog.upsertSupplier).handler(({ input }) => this.svc.upsertSupplier(input))
  }
  ...
  upsertCost(@OwnsReply() _reply: unknown) {
    return implement(contract.tenantCatalog.upsertCost).handler(({ input }) => this.svc.upsertCost(input))
  }
}
