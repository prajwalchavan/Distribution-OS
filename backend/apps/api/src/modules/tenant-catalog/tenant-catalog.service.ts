...
import { brands, manufacturers, productVariants, products, suppliers, tenantProductCosts, tenantProducts, withTenant, type Db } from '@dos/db'
import { BACK_OFFICE, currentTenant, DB, idempotent, requireDb, requireRole, STAFF } from '../../platform/index.js'
  ...
  async list(input: ListIn): Promise<ListOut> {
    ...
    return withTenant(db, ctx, async (tx) => {
      ...
      const joined = input.listedOnly
        ? base.innerJoin(tenantProducts, and(eq(tenantProducts.variantId, productVariants.id), eq(tenantProducts.tenantId, ctx.tenantId)))
        : base.leftJoin(tenantProducts, and(eq(tenantProducts.variantId, productVariants.id), eq(tenantProducts.tenantId, ctx.tenantId)))
    ...
    return withTenant(db, ctx, (tx) =>
      idempotent(tx, input.idempotencyKey, input, async () => {
        ...
        await tx
          ...
          .onConflictDoUpdate({ target: [tenantProducts.tenantId, tenantProducts.variantId], set: { ...values, updatedAt: new Date() } })
        const [item] = await tx
          .select(listingColumns)
          .from(productVariants)
          .innerJoin(products, eq(products.id, productVariants.productId))
          .innerJoin(manufacturers, eq(manufacturers.id, products.manufacturerId))
          .leftJoin(brands, eq(brands.id, products.brandId))
          .innerJoin(tenantProducts, and(eq(tenantProducts.variantId, productVariants.id), eq(tenantProducts.tenantId, ctx.tenantId)))
