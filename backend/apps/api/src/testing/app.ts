 ...
 */
export async function bootTestApp(imports: NonNullable<ModuleMetadata['imports']>): Promise<NestFastifyApplication> {
  const moduleRef = await Test.createTestingModule({ imports: [ORPCModule.forRoot({}), DbModule, ...imports] }).compile()
...
function toQuery(payload?: Record<string, unknown>): Record<string, string> {
  ...
  for (const [k, v] of Object.entries(payload ?? {})) if (v !== undefined) q[k] = String(v)
  return q
}
