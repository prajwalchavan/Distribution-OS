 ...
 */
export async function idempotent<T>(tx: Db, key: string, request: unknown, fn: () => Promise<T>): Promise<T> {
  ...
  if (inserted.length === 0) {
    ...
    if (!existing) throw new ORPCError('INTERNAL_SERVER_ERROR', { message: 'idempotency row vanished' })
    if (existing.requestHash !== requestHash) {
      throw new ORPCError('CONFLICT', { message: 'idempotencyKey was already used with a different request' })
    }
