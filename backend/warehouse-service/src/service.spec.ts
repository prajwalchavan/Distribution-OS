...
describe('warehouse-service', () => {
  ...
  it('answers /health and publishes its OpenAPI document with only its own routes', async () => {
    ...
    expect(spec.info.title).toContain('Warehouse service')
  ...
  it('documents bearer auth: securityScheme, x-roles per operation, no security on public routes', async () => {
    ...
    expect(me?.['x-roles']).toContain('warehouse')
  ...
  it('refuses roles this service does not serve, before any business logic', async () => {
    ...
    const denied = await app.inject({
      method: 'GET',
      url: '/tenancy/me',
      headers: await bearer({ tenantId, actorId, role: 'manager' }),
    })
    expect(denied.statusCode).toBe(403)
    expect(denied.json<{ message: string }>().message).toBe(
      'warehouse-service does not serve the manager role',
    )

    const allowed = await app.inject({
      method: 'GET',
      url: '/tenancy/me',
      headers: await bearer({ tenantId, actorId, role: 'warehouse' }),
    })
    // The gate let the warehouse through; with no membership for the fake ids the handler itself answers 401 (oRPC).
