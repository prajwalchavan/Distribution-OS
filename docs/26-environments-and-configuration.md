...
| Domain `<domain>` | **`distributionos.in`**, and the website is **`www.<domain>`** (founder, 2026-09-21, docs/22 §8) | `api.<domain>` is the VM behind Caddy — nobody types it, so it stays. `www.<domain>` is Pages, and the bare `distributionos.in` redirects to it. This replaces `app.<domain>` and `app-test.<domain>` wherever §5's table of 2026-09-05 still says them; that table is left as the dated record it is. The store bundle id `in.distributionos.app` is unrelated and unchanged — it is the phone's package name, never shown to a person. |
