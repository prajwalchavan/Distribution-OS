...
real `run:android` build signed in on the Pixel 7, iOS boots and renders in Expo Go.** The
independent pass ran the chain from the outside — `pnpm install` left the lockfile untouched, `lint`
**11/11**, `typecheck` **11/11**, `build` **8/8**, `test` **3/3 · @dos/ui 196 · @dos/api-client 63 ·
@dos/offline 38**, `format:check` clean in both workspaces, `docs:readme:check` clean,
native-resolution `tsc --noEmit` (with `customConditions: ["react-native"]`, so `@dos/ui` resolves to
`index.native.ts`) clean, and no `react-native` / `react-dom` / `@dos/ui/web` / `@dos/ui/native`
import and no raw hex in any app file — then drove the app in a real headless Chrome against the
founder's own database and the live auth :3000 / retailer :3006: **16 routes at 1440 × 900 and the
same 16 at 375 × 812, every screen measured rather than eyeballed**, ending with zero console errors,
zero warnings, zero non-2xx, zero horizontal scroll, minimum font 14 px, IBM Plex Sans the only
family, no cost / margin / landed word, no snake_case machine value and no uuid on any screen, and —
at the last pass — **not one tap target under its floor on either shell** (69 dp field, 32 px desk).
All thirteen screens of docs/23 §6.1 are there and answer with real Tarsun, Sai and Kalyan data.

**Fourteen defects found and fixed — five of them in the shared kit, so the other six apps inherit
the fixes.**

1. **THE SHOP WAS TOLD IT HAD NOT SENT THE ORDER IT HAD JUST SENT.** Measured end to end on the
   two-tap reorder: `setLines` **200**, `submit` **200**, and the screen the app then navigates to
   read **"Not sent yet" · "Not sent"** with a live **"Send this order"** button under an order the
   distributor had already accepted. The cache invalidates by key PREFIX; the editor and the detail
   screen share the key `['order', id]`, and `['orders']` does not prefix it — so the detail screen
   served the cached DRAFT for its 30-second freshness window. `create`, `setLines` and `submit` now
   invalidate `['order']` as well. The confirmation was dead too (`setToast` ran AFTER
   `router.replace`, on a screen being unmounted): the editor lands on `/orders/<id>?placed=1` and
   the detail screen says **"Order SO-0075 is with Sai Distributors, Dombivli."**
2. **A BILL SCREEN THAT PRINTED SOMEBODY ELSE'S PAYMENTS AS ITS OWN.** "WHAT YOU HAVE PAID" listed
   `receipts.list({ retailerId })` — the shop's last eight receipts, whatever they were against.
   Measured on INV/0031 (₹13,554, paid in full on 27 August): eight receipts, **not one of which
   touched that bill** — five ₹1 receipts from 5 September, a −₹13,554 reversal, a −₹40 and a +₹40,
   adding to **minus ₹13,549** — while the receipt that actually paid it was not listed at all. The
   panel now states the invoice's own figures ("Paid against this bill ₹40.00 · Still to pay ₹55.00",
   which agree with the bottom bar and the total) and sends the reader to Receipts for the rest.
   Naming which receipt paid which bill needs a backend gap closed (below).
3. **"LAST BILL" NAMED A BILL NINE DAYS OLDER THAN THE SHOP'S LAST BILL.** `invoices.list` pages by
   `id DESC` and takes no `orderBy`; for seeded, imported or back-dated rows an id is not a date, so
   `limit: 1` answered INV/0031 of 27 Aug while the shop's actual last bill was INV/9125 of 5 Sep.
4. **"MY BILLS", "MY ORDERS" AND THE INBOX WERE IN NO ORDER AT ALL** — same root cause on three
   lists. Bills opened 27 Aug · 26 Aug · 28 Aug · 5 Sep; orders 4 Sep · 5 Sep · 5 Sep · 6 Sep · 5 Sep;
   the inbox — the one screen this app exists to be the detail view of — opened on 26 August with
   this morning's WhatsApp messages twelve rows down. Each page is now ordered by the date a shop has
   actually seen (invoice date, submitted-at, sent-at).
5. **THE ACCOUNT SCREEN LISTED FORTY-ONE DEVICES WITH NO DATE ON ANY OF THEM.** `<Register>` in
   `field` density draws the identity, one `value` and one `chip` column and DROPS every `detail`
   column — and this app is `field` at every width, so the kind, the signed-in time and the last-used
   time were never drawn on the one screen whose only question is which session to end. Rewritten as
   rows ("Safari on Mac · Web · signed in 7 Sep, 4:38 pm · last used 7 Sep, 4:40 pm"). Same defect
   the statement had; `textColumn()` now carries the warning in its doc comment.
6. **KIT: THE DESK RAIL CUT THE DISTRIBUTOR'S OWN NAME THROUGH THE MIDDLE OF A LETTER.** `<Txt>`
   picks its scale by DENSITY, and the four field apps also open on a laptop, where they draw the
   same 172 px rail: the name came out at `field.title` 20/26/700, needing 95 px in an 88 px box that
   is `overflow: hidden` with no ellipsis, so the pilot's rail head read **"Tarsun Enterpris"**.
   UX-00 §8.1 sets that line at `desk.railTitle` (14/18/700), which is what `size="rail"` now renders
   in every density — the name fits whole, and a longer one ("Sai Distributors, Dombivli") ends in an
   honest ellipsis with the full name on hover. **This was live in the sales, warehouse and delivery
   apps too.**
7. **KIT: THE MARK WAS SIZED BY THE APP INSTEAD OF BY THE SHELL.** `<TenantSwitcher>` asked for the
   28 px rail box on the web in BOTH shells and the 32 dp header box on native in both. It now reads
   the viewport (`useViewport().kind`), which is the rule UX-00 §11 states: 28 px in the desk rail,
   32 dp in the phone header, with the type that goes with each.
8. **KIT: THE SWITCHER — THIS APP'S WHOLE SPINE — WAS A 52 dp TARGET.** `minHeight: size.desk` (32)
   is a literal, and on a phone the content alone is 52 dp: under every floor UX-00 §5.2 names, on
   every screen, on the control a shop that buys from three distributors uses constantly. It reads
   `theme.touchSize` now (69 dp measured here, 32 px on the desk rail), which is the rule `<MenuRow>`
   states in its own comment two functions below.
9. **KIT: A SEGMENT WAS SIZED BY ITS WORD.** "All" measured **54 dp** wide against a 69 dp floor on
   My orders and My bills. `minWidth` is the floor now, in both renderers, with a test.
10. **"1 items"** on the order editor and the order detail.
11. **THE STATEMENT CALLED A WRITE-OFF A PAYMENT AND A REVERSED RECEIPT A BILL.** The debit/credit
    labels were "Billed" / "Paid", so a reversed receipt read "Payment · **Billed** ₹40.00" and a
    write-off read "Written off · **Paid** ₹40.00" — telling a shopkeeper they had paid money they
    never paid. They say what the entry did to the balance: **Added** / **Taken off**.
12. **THE INBOX SAID "Sent by Sms" AND "Sent by On WhatsApp".** The channel words are names now
    (WhatsApp, SMS, the app, email), so the sentence reads.
13. **A DATABASE ID AS A SENTENCE A SHOPKEEPER READS.** `orders.repeatLast` stores `Repeat of <the
previous order's uuid>`, and the order detail printed it under "Your note": "Repeat of
    fd62e4e0-f68c-7e07-bfa2-6478bed9282f". It reads "A repeat of an earlier order"; the server half is
    a gap below.
14. **THE ORDER SCREEN OPENED ON AN EMPTY TEXT BOX.** With nothing chosen the two panels above it
    render nothing, so "Anything to tell them" floated to the top and a shop opening "Place an order"
    on a phone spent its first screen on a note field (UX-00 §8.2: open on the most likely next
    action). The note appears once there is an order to say something about. On the way: an unordered
    row's line total drew `<Money value={null}>` — an em dash at the top right of the row, next to the
    stepper's own "−", on a row that already says "Not ordered".

**What was proved, not assumed.** **Two-tap reorder end to end**, twice: `orders.repeatLast` → the
editor with the shop's last basket re-priced today → "Place order" → `setLines` + `submit` → **SO-0075
on the server**, the detail screen reading "Accepted", and the confirmation naming the distributor.
**Distributor switching**: `ramesh.gupta` holds three memberships; opening Sai Distributors re-scopes
the shop, the dues (₹7,065.64), the orders, the bills and the chrome — Tarsun's lorry never appears
over Sai's or Kalyan's name (their own initials mark does). **Role boundary**: `sunil.tarsun` (owner)
gets the service's own sentence as the whole screen with **zero calls to :3006** — measured, the only
requests are the two to `/auth/login`. **Honesty**: with :3006 blocked and auth alive, every figure
becomes an em dash under "Offline since 5:44 pm" and "No connection. This is not the current
picture." — never a stale rupee presented as current. **Prices**: the price list is quoted through
`pricing.quote` — the same `priceOrder()` engine that prices the order and the bill — so a rate on
the shelf is the rate on the bill, with "your rate, list ₹14.29" where the shop's tier, override or
agreed rate beats the list.

**Android**: a real `expo run:android` debug build (`in.distributionos.retailer`) installed on
`Pixel_7_API_36`, signed in with `adb input`, walked to Home, My bills and Place an order — the
distributor's logo and name in the header, ₹1,901.00, the ⋯ sheet with twelve destinations and no
duplicate, bills newest-first, IBM Plex Sans throughout. **iOS**: boots and renders in Expo Go on the
iPhone 16 Pro / iOS 18.0 simulator (`xcrun simctl openurl … exp://127.0.0.1:8081`, screenshot with
`xcrun simctl io`); there is still no headless input path on iOS, so it is verified as far as the
sign-in screen, as in every gate before this one.

**Environment fixed on the way (founder autonomy, 2026-09-06): `brew install openjdk@21`.** Android
Studio now bundles **JDK 25**, whose "WARNING: A restricted method in java.lang.System has been
called" is written to stderr and AGP's `GeneratePrefabPackages.reportErrors` turns any stderr line
into a build failure — `:react-native-worklets:configureCMakeDebug` failed three times, on a clean
tree, with that as the entire message. **Every Android build from now needs
`export JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home`**, not Android
Studio's bundled jbr; CLAUDE.md's "Local environment" line is now out of date on that point.

**Backend gaps this gate found (recorded, not worked around).**

(1) **No list is orderable.** `billing.invoices.list`, `orders.list` and `notifications.messages.list`
all page by `id DESC` and take no `orderBy`. Ids are UUIDv7 for rows this product issues, so
production reads right — but a seeded, imported or back-dated row sorts anywhere, and the app can only
sort the page it was given. An `orderBy` (or ordering by the document's own date) would make the
cursor and the reading agree.

(2) **A bill cannot name the money that paid it.** `receivables.receipts.list` has no `invoiceId`
filter, there is no `allocations.list`, and `billing.invoices.get` carries `creditNotes` but not its
allocations. Either an `invoiceId` filter on `receipts.list` or `allocations` on `invoices.get` would
let R4 show the receipts against that bill instead of a figure.

(3) **`orders.repeatLast` writes a uuid into a shop-visible field** (`note: 'Repeat of ' + previous.id`,
`orders.service.ts:206`). The order NUMBER would read; the app hides the id in the meantime.

(4) **iOS has no headless input path** and `expo run:ios` is still blocked by the 18.2 SDK / 18.0
runtime mismatch — unchanged from the five gates before this one.

**Local links.** Retailer app <http://127.0.0.1:5178> (`pnpm --filter @dos/retailer-app web` in
`frontend/`, or the `retailer-app` entry in `.claude/launch.json`) against auth-service :3000 and
retailer-service :3006. Sign in **`ramesh.gupta`** / `Dos@1234` — the shop that buys from all three
distributors (`fatima.shaikh` holds two). Delivery app <http://127.0.0.1:5177> (`ganesh.more`),
warehouse <http://127.0.0.1:5176> (`dinesh.patil`), sales <http://127.0.0.1:5175>
(`rahul.deshmukh`), manager <http://127.0.0.1:5174> (`vikas.kadam`, `meena.joshi`), owner
<http://127.0.0.1:5173> (`sunil.tarsun`) — every password `Dos@1234`.

**NEXT: the admin app** (`frontend/admin-app`, the platform console on :3007, docs/23 §7 and the
module 13 row below) — it is a placeholder package today. Then the final end-to-end gate across all
seven apps.

---

## RESUME HERE (updated 2026-09-07 14:10 IST, session 4)

**FRONTEND SLICE 6 `delivery-app` GATED AND GREEN (2026-09-07 14:10 IST) — web at both shells,
Android on the Pixel 7 emulator with a `run:android` build, iOS boots in Expo Go.** The independent
pass ran the chain from the outside — `pnpm install --frozen-lockfile` clean and the lockfile
untouched, `lint` **10/10**, `typecheck` **10/10**, `build` **7/7**, `test` **3/3 · @dos/ui 190 ·
@dos/api-client 63 · @dos/offline 38**, `format:check` clean, native-resolution `tsc --noEmit` clean,
no `react-native` / `react-dom` / `@dos/ui/web` / `@dos/ui/native` import in any app file — then drove
the app in a real headless Chrome against the founder's own database and the live auth :3000 /
delivery :3005: **16 routes at 1440 × 900 and the same 16 at 375 × 812, every screen measured rather
than eyeballed, ending with zero console errors, zero warnings and zero non-2xx.** All twelve screens
of docs/23 §5.1 are there and answer with real Tarsun data.

**Ten defects found and fixed — four of them outside this app (two in `@dos/ui`, one in
`@dos/offline`, one in the app template), so the remaining apps inherit them.**

1. **A PHONE THAT ALREADY HELD THE TRIP SAID "Nothing is on the road yet".** `@dos/offline`'s
   `queryTable` answers `[]` for a table whose SHAPE it does not know yet, the shapes arrive with
   `restoreManifest`, and `useTable` re-runs only on a table CHANGE — so a screen that mounted before
   the engine was ready asked once, got nothing, and kept nothing. On a device whose store is already
   full and whose delta pull has nothing new to bring, **no change ever comes**: measured on the Pixel
   7, the delivery home screen said "Nothing is on the road yet" on **every cold start**, over a
   SQLite file (pulled off the device and read) holding TRIP-NEXT as `active` among 131 trips. That is
   the offline-first app failing at exactly the moment it exists for — a driver opening the app in the
   morning with no signal. Becoming able to answer is itself a change now, and a regression test in
   `engine.test.ts` fails without the emit. **This was latent in the sales and warehouse apps too.**
2. **THE APP DISAGREED WITH ITSELF ABOUT WHICH TRIP THE CREW IS ON.** `pickCurrentTrip` runs over
   whatever the device holds, and during the first pull that is a subset: measured, `/` named
   TRIP-NEXT while `/trip/start` and `/expenses` named TRIP-ACTIVE for the twenty seconds the pull
   took — with a live "Start the trip" under the wrong one and an expense form that would have posted
   the diesel to yesterday's trip. `useHydrated()` was FALSE for all of it and no screen said so. D1,
   D2, D7 and D8 now carry the sentence above the content (never over it), and the two screens that
   WRITE against the guess wait for the pull — both already need a signal, and the pull runs on it.
3. **THE DAY SUMMARY TOLD THE DRIVER TO HAND OVER ₹2,500 LESS THAN THE PHONE HELD.**
   `trips.settlementPreview` adds up `collections`; a doorstep receipt taken with no signal goes back
   as a `receipts` op (docs/23 §5.4 — `collections` is not a writable sync table), reaches receivables
   correctly and writes **no** collections row. Measured on TRIP-NEXT: the office said cash ₹5,000, the
   phone held receipts for ₹7,500 on the same trip, and D8 said "Hand ₹11,070.00 to the cashier". It
   now names the difference and hands over ₹13,570. The backend half is an open point below.
4. **"Van sales are switched off for this trip" WAS A LIE THE SCREEN COULD NOT BACK UP.**
   `bool(undefined)` turned "the trip has not reached this device yet" into a definite refusal, on a
   trip whose `van_sales_enabled` is true under a feature flag that is on. `null` is the third answer.
5. **THE STOP HEADER COUNTED THE STOPS THAT HAD ARRIVED, NOT THE STOPS ON THE TRIP** — "Stop 3 of 3"
   on a six-stop day, and "0 of 1 stops done" on D1. The trip's own `planned_stops` now answers.
6. **"Your trips" WAS NEITHER THE LAST 30 DAYS NOR IN ORDER, AND DID NOT CONTAIN TODAY'S TRIP.**
   `trips.list` was called unwindowed at `limit: 50` and the server pages by `id DESC`, so the
   founder's data answered 17 Jan 2027, 9 Aug 2027, 10 Jan 2027 … under a chip reading "Last 30 days",
   with the crew's own active trip absent. `from`/`to` are on `TripsListInput`; the page is sorted
   newest-first here. **And every row opened `/` — today's trip** — so a crew member checking Tuesday
   landed on Thursday's road; rows now open that trip's own day summary (`/day?tripId=`, which falls
   back to the office's `trips.get` for a trip the phone does not hold, and reads in the past tense).
7. **A DRIVER'S UNSENT WORK WAS INVISIBLE WHILE OFFLINE.** `<ConnectionStrip>`'s offline branch won
   outright and dropped the pending count — measured with one arrival queued at a shop door, the strip
   said only "Offline since 12:01 pm". Both facts on one line now, in both renderers, with two tests.
8. **ANDROID ASKED FOR A LOCATION POWER THE BUILD CANNOT USE, AND THE REFUSAL ESCAPED.**
   `requestPermission({ background: true })` with no `ACCESS_BACKGROUND_LOCATION` in the manifest and
   no `expo-task-manager` task anywhere in the repo: a red "Uncaught (in promise) … You need to add
   `ACCESS_BACKGROUND_LOCATION` to the AndroidManifest" over the home screen, and no tracking at all.
   The app asks for the foreground grant it actually uses; the kit turns a missing manifest entry into
   `background: false` instead of a rejection; every other location failure (Play Services' own
   "unsatisfied device settings", which also escaped) ends on the screen's own sentence.
9. **D1 PROMISED SOMETHING THE PHONE DOES NOT DO** — "A browser tab stops sending when it is hidden.
   **The phone app does not.**" Nothing in this repo registers a background task. Each platform now
   says its own true thing, on both.
10. **THE PHONE'S ONLY MENU LISTED TWO SCREENS TWICE.** With no tab bar `AppShell` merges the nav
    sections and the account items into one ⋯ sheet: measured on the Pixel 7, Today · Day summary ·
    Expenses · Attention · Trip history · Me · **Me** · **Your trips** · Change your password.
    Also fixed on the way: D4's chip printed a green "Delivered" over a bill nobody had recorded yet
    (it names the tense now), D5's bottom bar labelled the stop's ₹9,399 "The shop owes" beside a chip
    reading "Owes ₹68,203.00", and `<RupeeInput>`'s 69 dp control had a 67 dp `<input>` inside its
    border — the whole control takes the tap now (measured: an edge tap focuses the field).

**What was proved, not assumed.** Offline: airplane mode at a shop door → "I am at the shop" → the
tray reads **"Waiting: 1 · Stop · 7 Sep, 12:01 pm"** → signal back → `trip_stops.state = 'arrived'`
**on the server with the driver's own tap time**, 12:01:19, and a `sync_ops` row. Service dead
(:3005 blocked, auth alive): "Offline since 12:04 pm" and "Still filling this phone from the office"
on every panel — never a stale figure under "Updated just now". Role boundary: `vikas.kadam` gets the
service's own sentence as the whole screen with **zero further calls**. No cost, margin or landed
figure anywhere in the DOM or in any payload this role can fetch. Phone shell: **no tab bar**
(UX-00 §8.2), min font 14 px, IBM Plex Sans throughout, no horizontal scroll at 375 px, and the only
sub-69 dp target is the money field's inner `<input>` inside a 69 dp control.

**Android** was built with `expo run:android` (**BUILD SUCCESSFUL in 20m 13s** on JDK 17 —
`~/.gradle/jdks/eclipse_adoptium-17-aarch64-os_x.2`), installed on `Pixel_7_API_36` and driven with
`adb`: sign-in, D1 (TRIP-NEXT · 5 Sep, still to collect ₹46,414.00, next stop Om Sai ₹9,399.00),
D3 (Stop 1 of 6, owes ₹68,203.00, 4 bills open), D8 (Hand ₹13,570.00 with the uncounted-receipts
sentence), D11 and D12 — all matching the browser, with **zero `ReactNativeJS` errors** in logcat
after the fixes. **iOS** boots and renders in Expo Go on the iPhone 16 Pro / iOS 18.0 simulator
(screenshot), and cannot be signed in headlessly on this Mac — unchanged from the last four gates.

**Carried items for the next session.**

(1) **`collections` cannot be reached by a device.** It has an upload handler in `delivery.sync.ts`
but is absent from `SYNC_PULL_TABLES`, so `SyncRegistry.manifest()` can never mark it writable and
`engine.enqueue()` refuses it by name. A doorstep receipt taken offline therefore reaches receivables
as a receipt and never becomes the trip's collection, and `trips.settlementPreview` under-reports the
cash by exactly that much. The app now says so; the fix is one line in `sync-tables.ts` plus the
guarantees migration. Same for `trip_expenses` and `pod_evidence`.

(2) **`delivery.trips.list` has no date ordering.** It pages by `id DESC`, which for seeded rows is
arbitrary; the app sorts the page it fetched. An `orderBy` (or ordering by `trip_date DESC, id DESC`)
would make the cursor and the reading agree.

(3) **A dev-only LogBox error remains on Android**: "Can't perform a React state update on a
component that hasn't mounted yet", component stack `<ContextNavigator/>` → `<ExpoRoot/>` → `<App/>`,
i.e. inside expo-router's own tree. It does not reproduce on the web (zero console errors on 32 route
walks) and no app component is named. The redirect is navigator-gated AND now deferred out of the
commit and skipped when it would be a no-op (in the app and in the template); the warning survives
both. Worth one focused session with the React DevTools profiler on a phone.

(4) **`expo run:ios` is still blocked** by the iOS 18.2 simulator SDK / 18.0 runtime mismatch, and
**iOS still has no headless input path** (`simctl` has no input verb, `idb-companion` needs macOS 26,
the panel is off limits) — unchanged from the owner, manager, sales and warehouse gates.

(5) **The web offline store is memory-only**, so a browser re-pulls the whole read set on every
reload; on the delivery data that is ~20 s before the right trip is named. Honest on screen; COOP/COEP
headers on the web build would fix it.

**Local links.** Delivery app <http://127.0.0.1:5177> (`pnpm --filter @dos/delivery-app web` in
`frontend/`, or the `delivery-app` entry in `.claude/launch.json`) against auth-service :3000 and
delivery-service :3005. Sign in **`ganesh.more`** / `Dos@1234` (delivery at Tarsun; `santosh.kamble`,
`iqbal.shaikh`, `raju.yadav` are the other crew, `vijay.salvi` at Sai, `nikhil.waghmare` at Kalyan).
Warehouse app <http://127.0.0.1:5176> (`dinesh.patil`), sales app <http://127.0.0.1:5175>
(`rahul.deshmukh`), manager app <http://127.0.0.1:5174> (`vikas.kadam`, `meena.joshi`), owner app
<http://127.0.0.1:5173> (`sunil.tarsun`) — every password `Dos@1234`.

**NEXT: the retailer app** (`frontend/retailer-app`, docs/23 §6) — `pnpm --filter @dos/app-template
new retailer 5178`, then `pnpm docs:readme` in `backend/` in the SAME turn. It is the shopkeeper's
app: online only, `field` touch, and the only one whose role may hold more than one distributor.

---

## RESUME HERE (updated 2026-09-07 09:10 IST, session 4)

**FRONTEND SLICE 5 `warehouse-app` GATED AND GREEN (2026-09-07 09:10 IST) — web at both shells,
Android on the Pixel 7 emulator, iOS boots in Expo Go.** The independent pass ran the chain from the
outside — `pnpm install` leaves the lockfile untouched, `lint` **9/9**, `typecheck` **9/9**, `build`
**6/6**, `test` **3/3 · @dos/ui 184 · @dos/api-client 63 · @dos/offline 37**, `format:check` clean —
then drove the app in a real headless Chrome against the founder's own database and the live
auth :3000 / warehouse :3004: **20 routes at 1440 × 900 and the same 20 at 375 × 812, every screen
measured rather than eyeballed, ending with zero console errors, zero warnings and zero non-2xx.**
All twelve screens of docs/23 §4.1 are there and answer with real Tarsun data.

**Eight defects found and fixed — five of them in the shared kit, so the remaining apps inherit the
fixes.**

1. **THE PICKING SHEET HID THE 23 ROWS IT WAS HOLDING.** `LocalAsync` drew "Still filling this phone
   from the server" INSTEAD of the device's rows whenever the read set was not yet complete. Measured
   on PICK-0011 at both widths: the header said "1 of 23 picked", the bottom bar said "1 of 23
   picked" and "22 lines not yet picked", and **not one of the 23 rows was drawn** — over data the
   screen had already counted twice. On the web the offline store is the memory adapter (no
   cross-origin isolation), so the whole read set is re-pulled on every reload — 154 pages and ~70 s
   on the pilot's data — and for all of it the one screen a picker holds was blank. The sentence now
   sits ABOVE the rows instead of over them; "nothing here" and "not everything is here yet" stay two
   different sentences. Same fix repaired W6's "WHAT GOES IN THE CARTONS".
2. **THE VAN CHECK-IN CALLED PIECES CASES.** `StockBalanceRow` carries no pack (the balance is
   pieces) and W9 printed it through `caseLine(pcs, 1)`, so **201 pieces of Balaji Chataka Pataka
   Wafers — a 48-piece case — read "201 cs = 201 pc"** to the loader counting them back off the
   vehicle. The pack now comes from the device's own lot (`useLotCaseSize`), and where the device
   cannot answer the screen says pieces WITH THEIR UNIT rather than a case count it cannot back up.
   W8's three bare figures ("On hand 41 · Held 41 · Free 0") carry their unit for the same reason.
3. **THE VARIANT-NAME INDEX CAPPED AT 5,000 ROWS AGAINST A 5,768-ROW CATALOGUE.** `product_variants`
   is the GLOBAL curated catalogue, not this distributor's 29 listings, and it was read whole under a
   limit. The 768 rows outside the window had no name: measured on the Pixel 7, the gate count's title
   read **"Item not on this phone yet" over "Too Yumm Makhana Himalayan Salt 20 g"** — invisible on
   the web only because SQLite returned the pages in a different order. It is the same page-one bug
   the earlier `tenantCatalog.list({ limit: 500 })` version had with a bigger number in front of it.
   Names are now read BY ID, so a screen asks for the few dozen it needs.
4. **`<NumberPad>`'s ACTION BUTTON RENDERED AT THE BROWSER'S DEFAULT 13.33 px** — a raw `<button>`
   with no type token, under the 14 px floor of UX-00 §4.3 and nowhere near the 16 sp its native twin
   has always used. It is the button under both BLIND counts ("Damaged pieces" on the gate count,
   "Done" on the load sheet). It is the kit's own `<Button>` now, and `.dos-btn` carries a 14 px floor
   so the next raw one cannot fall through either.
5. **`<NumberPad>`'s "expected" WAS ALWAYS DRAWN AS MONEY.** `NumberPadProps` says `value` is paise in
   `money` mode and a count in `count` mode; `expected` was rendered through `<Money>` in both, so the
   van check-in's expected van stock would have printed **₹2.01 for 201 pieces**. Fixed in both
   renderers, with tests.
6. **THE WEB `<Chips>` SHIPPED A 9.5 px GAP WHERE THE NATIVE HALF SHIPPED 25.** UX-00 §6.10 puts
   filter chips "≥ 19 dp apart (25 dp warehouse)"; the native half always read it that way and this
   one wrote `gap.adjacent / 2` for all seven apps. Measured at 375 px on the level-2 row (Queues ·
   Capture · Stock · Counts · Held): **10 px** between chips a gloved thumb has to choose between.
   Now touch-aware in both renderers, with the pad's key grid (12 px → 25 dp on a warehouse screen),
   the `<Screen>` bottom bar's clearance from the phone tab bar (13 px → 25 dp), and the eight
   two-button rows in this app's own screens (16 px → 32 px).
7. **A 401 ON EVERY COLD START.** The access token is deliberately never persisted, and the sync
   engine was started the instant a restored session appeared — so `GET /sync/manifest` went out with
   no bearer and took a **401 on all twenty routes**, one console error and one wasted round trip per
   launch, recovered by the client's own transparent refresh, which is exactly why nobody saw it. The
   engine now waits for the token; the provider still stays mounted through the refresh. The final
   walk is **0 non-2xx**.
8. **ANDROID DREW ITS STATUS BAR IN WHITE ON A WHITE APP.** `userInterfaceStyle: "light"` tells the
   app which palette to use and tells Android nothing; measured on the Pixel 7, the clock, signal and
   battery were invisible over the app's own ground. `expo-status-bar` was a dependency of every app
   and used by none. Fixed here and in `libs/app-template`, so delivery, retailer and admin are born
   correct. (Fixing the pad's gap also exposed a latent native bug: three `width: '30%'` keys plus two
   25 dp gaps do not fit a wrapping row, so the keypad silently became **1 2 / 3 4 / 5 6** — it is
   three flex rows of three now, at any gap.)

**What was verified, not assumed.** Offline (CDP network kill) on the picking sheet: the strip reads
"Offline since 8:20 am", the sheet says "Picks are saved on this phone and sent when there is a
signal", the pick lands locally ("2 of 23 picked"), and when the signal returns the tray drains to
"Nothing is waiting" and **`pick_lines.picked_qty_pcs = 24` with `picked_at 08:20:42` is on the
server** — outbox to Postgres, end to end. Service stopped (`:3004` killed, then restarted): the strip
says "Offline since 8:52 am", every KPI is an em dash rather than a false zero, and every panel says
so — the read sentence itself was wrong ("No connection. This will send when the signal is back." on a
panel that was asking, not sending) and now reads "No connection. This is not the current picture."
Role boundary: `vikas.kadam` (manager) signs in and gets the service's own sentence as the whole
screen, one button, **zero further calls and zero non-2xx**. Cost boundary: `tenantCatalog.costs` and
`reporting/registers/stock-value` both **403**, the warehouse manifest is 13 tables with no cost,
margin or landed column, and the word "cost" appears in this app only in comments. Design system on
every screen: **min font 14 px, IBM Plex Sans throughout, no horizontal scroll at 375 px**.

**Android** was walked headlessly on `Pixel_7_API_36` (`adb shell input`, `adb exec-out screencap`):
sign-in, W1 queues, W4 fulfilment queue, W5 picking sheet and W3 gate count all render with real data
in the native shell. **iOS**: the app boots and renders in Expo Go on the iPhone 16 Pro / iOS 18.0
simulator (screenshot), but signing in needs a tap and this Mac has **no headless input path** —
`xcrun simctl` has no input verb, `idb-companion` refuses to install (it requires macOS 26 / Xcode 26;
this Mac is macOS 14.6.1), and the two remaining options are both off limits (the founder's
2026-09-06 rule: no iOS panel on their screen). Android exercises the same native renderer.

**Carried items for the next session.**

(1) **Chips are narrower than the floor even though they are as tall as it.** UX-00 §6.10 fixes chip
HEIGHT at the app's floor and says nothing about width, so "Stock" (68 × 76), "Held" (61 × 76) and the
ghost "Clear" (57 × 76) pass the rule as written and still measure under 12 mm across. Worth a founder
decision on whether the floor is a height or a size.

(2) **The gate-count pad scrolls on a phone.** 4 rows of 76 dp keys, 25 dp gaps, the value display and
the action do not fit 375 × 812; measured 632 px of pad in a 418 px window. Everything is reachable
(0 / Clear / ⌫ and the action all sit above the bottom bar once scrolled) but UX-00 §6.3 calls this
"the full-screen pad". Either the pad's own action moves into the screen's bottom bar or §6.3 admits
the scroll.

(3) **The four home-strip figures are floors, not counts** ("Bills to count 20+", "Waves open 40+").
None of the list procedures answers a total — they answer a page and a cursor — so the app says "20+"
rather than dressing a page size as a count. A `total` on `procurement.grns.list`,
`warehouse.picklists.list`, `warehouse.packs.list` and `warehouse.loadSheets.list` would let the
godown see its real queue.

(4) **A counting GRN has no document number**, so twenty rows of the home queue read "Receipt at
Godown · 5 Sep, 10:32 pm" and are told apart only by the clock. `grn_no` is assigned at post; a
`supplierInvoiceNo` or a supplier name on `GrnListItem` would give the row an identity a hand can use.

(5) **The web offline store is memory-only** (`@dos/offline` needs cross-origin isolation for OPFS),
so a desk terminal re-pulls the whole read set on every reload — 154 pages, ~70 s on the pilot's data.
Honest on screen ("Held in memory only — a reload empties this device") and fine on the phone, where
SQLite is real. COOP/COEP headers on the web build would fix it.

(6) **`expo run:ios` is still blocked** by the iOS 18.2 simulator SDK / 18.0 runtime mismatch
(unchanged from the owner, manager and sales gates) and **CLAUDE.md's Android JDK line is still
stale** — the bundled JBR is JDK 25 and AGP needs 17.

**Local links.** Warehouse app <http://127.0.0.1:5176> (`pnpm --filter @dos/warehouse-app web` in
`frontend/`, or the `warehouse-app` entry in `.claude/launch.json`) against auth-service :3000 and
warehouse-service :3004. Sign in **`dinesh.patil`** / `Dos@1234` (warehouse; `kavita.sawant` is the
other hand at Tarsun, `bharat.jadhav` / `sunita.gaikwad` at Sai, `ravi.chormale` /
`manisha.palve` at Kalyan). Sales app <http://127.0.0.1:5175> (`rahul.deshmukh`), manager app
<http://127.0.0.1:5174> (`vikas.kadam`, `meena.joshi`), owner app <http://127.0.0.1:5173>
(`sunil.tarsun`) — every password `Dos@1234`.

**NEXT: the delivery app** (`frontend/delivery-app`, docs/23 §5) — `pnpm --filter @dos/app-template
new delivery 5177`, then `pnpm docs:readme` in `backend/` in the SAME turn. It is offline-before-pilot
with GPS, the `floor` touch size for the stop actions and `field` elsewhere (UX-00 §5.2).

---

## RESUME HERE (updated 2026-09-07 03:15 IST, session 4)

**FRONTEND SLICE 4 `sales-app` GATED AND GREEN (2026-09-07 03:15 IST) — web, iOS, and an Android dev
build that installs.** The independent pass ran the chain from the outside — `pnpm install` clean and
`--frozen-lockfile` clean, `lint` **8/8**, `typecheck` **8/8**, `build` **5/5**, `test` **3/3 ·
@dos/offline 37 · @dos/ui 167 · @dos/api-client 63**, `format:check` clean — then drove the app in a
real headless Chrome against the founder's own database and the live auth :3000 / sales :3003:
**18 screens at 1440 × 900 and 12 at 375 × 812, every one of them measured rather than eyeballed, with
zero console errors and zero non-2xx** outside the deliberate role probe. All fourteen screens of
docs/23 §3.1 are there and answer with real Tarsun data.

**Six defects found and fixed, three of them in the shared kit.**

1. **THE WHOLE ROW WAS NOT THE TAP TARGET, AND MONEY DID NOT FORM A COLUMN.** The web
   `<Pressable>` rendered a row-direction flex `<button>`, and a row-direction container does not
   stretch a child along the main axis — so the row inside sized to its content. Measured on _My
   orders_ at 1440 px: a **1168 px button with a 195 px row in it**, the money starting at x = 304 on
   a short row and **x = 708** on a row whose note ran long, and 973 px of every row dead to the
   touch (UX-00 §6.6). The native half is an `RNPressable`, i.e. a View — column, `alignItems:
stretch` — so the two renderers disagreed; the web half is now the same column. Two tests.
2. **THE BEAT SAID "No order yet" OVER SIX OF NINE SHOPS THAT HAD ORDERED.** `useMyOrders(200)`
   capped the rep's orders at the newest 200 and the beat worked out each shop's last order from
   that window; on the pilot data (442 orders on the device) the window ended inside 5 September, so
   every shop whose last order was 4 September was labelled as never having bought — while the
   shop's own card, three taps away, said "4 Sep". The line a rep uses to decide where the next ten
   minutes go, wrong in the direction that costs an order. Now `useOrdersForShops(beat)` — scoped by
   shop, not capped by count, and smaller as well.
3. **"Last order" claimed a shop-wide fact from a device that holds only the rep's own orders.**
   `sales_orders` is pulled scoped to the signed-in rep (docs/23 §3.4), so a shop another rep sold to
   three days ago read "No order yet" — which is exactly the reading that sends a rep past it. The
   beat and the shop card now say "your last order 6 Sep" / "No order from you yet".
4. **A MANAGER COULD SIGN IN AND THE APP NEVER SAID NO.** `auth-service` does not know which app
   asked, so `vikas.kadam` got the salesperson's rail drawn around an empty beat and the sentence
   "Still loading the beat onto this phone" **for ever**, while every call — the sync manifest
   included — answered `403 sales-service does not serve the manager role`. The service's own
   sentence is now the whole screen, the sync engine is not started, and the only button signs out.
   Proved on the web AND in Expo Go on the iPhone.
5. **`<Segments>` WAS 65 dp AGAINST THIS APP'S 69 dp FLOOR** on five phone screens (My orders,
   Catalog, Visits, AI drafts, Inbox): the button was `height - 4` so that the group's 2 px inset
   kept the OUTER pill at the floor, but the floor is about the thing a thumb hits. Fixed in both
   renderers with two tests. Every phone screen now measures 0 targets under 69 dp.
6. **A STOPPED SYNC ENGINE KEPT CRAWLING THE READ SET.** `stop()` cleared the timers and closed the
   database, but a pull loop already inside its `for(;;)` held the store it was handed and ran to the
   last page — **530 `sync/pull` calls measured on one sign-in where the read set is 265 pages**.
   Latent in normal use, real on a sign-out, a distributor switch or any re-mount during the two
   minutes a cold read set takes; on a field phone it is twice the data (UX-00 §8.3 budgets 10 MB a
   day) and a signed-out app still calling the service. One line in `pullLoop`, one in `flush`, and a
   test that fails without them.

**What was verified, not assumed.** The device prices the order with the server's own engine and
agrees to the paisa: ₹3,648.48 on the phone came back as `subtotal_paise 364848` on **SO-1115** in
Postgres. Offline (CDP network kill): the primary action becomes **"Save on this phone"**, the tray
reads "Waiting: 2 · No signal. Everything below stays on this phone until there is one", the strip
reads "Offline since 1:32 am"; back online it drains to "Waiting: 0" and the order is on the server —
as a **draft**, which is what `orders.sync.ts` allows and what the confirmation copy promises
("Submit it from My orders once it lands"). Service stopped mid-walk: the strip says "Offline since
1:43 am" and the screens keep showing what the device holds — never a stale number under "Updated
just now". Cost boundary: the salesperson manifest is **21 tables with no cost, margin or landed
column**, `tenantCatalog.costs` answers 403, and the word "cost" appears in this app only in comments.
Design system measured on every screen: **min font 14 px, IBM Plex Sans everywhere, 0 targets under
the floor, no horizontal scroll at 375 px**.

**iOS** was walked in **Expo Go on the iPhone 16 Pro / iOS 18.0 simulator** — sign-in, beat, shop card
and order entry, same content as the web with the native shell (bottom tabs, sticky footer). **Android**:
`expo run:android` is **BUILD SUCCESSFUL on JDK 17** and the APK installs and launches as
`in.distributionos.sales`; the emulator then died and would not come back on this Mac (see the
carried items), so the Android screens are not screenshotted this pass.

**Carried items for the next session.**

(1) **The seeded WhatsApp photo does not exist in object storage.** `inbound_messages.media_object_key`
`tenant/…/inbound/9241c99b…/photo.jpg` answers **404 `no object at …`**, the browser then ORB-blocks
the JSON error being loaded as an image, and S13 Inbox shows the browser's broken-image glyph under
"payment done, see screenshot". Backend/demo-data, not the app — but `<Img>` has no `onError` in the
kit contract, so no app can degrade gracefully until it does.

(2) **CLAUDE.md's Android JDK line is stale.** Android Studio's bundled JBR is now **JDK 25.0.3**, and
AGP's prefab step treats the JVM's `WARNING: A restricted method in java.lang.System has been called`
on stderr as a build error: every `configureCMakeDebug` task fails, in **owner-app too** (proved), so
it is a toolchain regression and not a sales-app defect. `JAVA_HOME=~/.gradle/jdks/eclipse_adoptium-17-aarch64-os_x.2/jdk-17.0.20.1+1/Contents/Home`
builds cleanly, which is what the owner and manager gates already used. CLAUDE.md should say JDK 17,
not "Android Studio's bundled one".

(3) **The Android emulator would not restart** after being stopped: it comes up on ephemeral IPv6
console ports instead of 5554/5555, logs `Failed to find ColorBuffer`, and `adb` never sees it
(`multiinstance.lock` cleared, ports free, three `-gpu` modes tried). Environment.

(4) **`expo run:ios` still cannot link on this Mac** — Xcode 16.2 ships only the iOS **18.2** simulator
SDK and this machine has the **18.0** runtime; `xcodebuild` offers no eligible destination. Unchanged
from the owner and manager gates; iOS is verified through Expo Go.

(5) **An order queued offline lands as a server draft and has to be submitted by hand** from My orders
once it lands (`orders.sync.ts` refuses any state past `draft`, by design). The copy says so at the
moment of saving, but after the queue drains the tray says "Nothing is waiting" with no pointer to the
drafts that still need a rep. Worth a founder decision.

(6) **Two leftover dev servers from earlier gates were serving the wrong app.** Metro on **8081** was
still the manager app's, so the sales dev client on Android loaded the manager bundle ("Distribution
OS - Manager" on a sales activity). Killed, along with the owner (:5173) and manager (:5174) web dev
servers. Start one Metro per app, and check `lsof -ti :8081` before blaming the app.

(7) **The demo data still carries a destructive smoke run.** `users.name` for `rahul.deshmukh` is
**"Demo Docs Staff (edited)"** in Postgres, so that is what the app's account chip and X4 print — the
app is right and the row is wrong. `pnpm db:seed` is idempotent and is the documented remedy
(CLAUDE.md), but it was not run here because it would move the very rows this gate's evidence points
at (SO-1115 and the offline draft on Ganpati General Stores).

**Local links.** Sales app <http://127.0.0.1:5175> (`pnpm --filter @dos/sales-app web` in `frontend/`,
or the `sales-app` entry in `.claude/launch.json`) against auth-service :3000 and sales-service :3003.
Sign in **`rahul.deshmukh`** / `Dos@1234` (salesperson; `amit.pawar`, `pooja.shinde` are the other two).
Manager app <http://127.0.0.1:5174> (`vikas.kadam`, `meena.joshi`), owner app <http://127.0.0.1:5173>
(`sunil.tarsun`) — every password `Dos@1234`.

**NEXT: the warehouse app** (`frontend/warehouse-app`, docs/23 §4) — `pnpm --filter @dos/app-template
new warehouse 5176`, then `pnpm docs:readme` in `backend/` in the SAME turn. It is the **76 dp** floor
app (UX-00 §5.2), phone + desk, and the first consumer of the gate-count keypad.

---

## Previously (updated 2026-09-06 23:00 IST, session 3)

**FRONTEND SLICE F4 `@dos/offline` + the kit's `<MapView>` GATED AND GREEN (2026-09-06 23:00 IST).**
The independent pass ran the chain from the outside — `pnpm install` clean and `--frozen-lockfile`
clean, `lint` **7/7**, `typecheck` **7/7**, `build` **4/4**, `test` **3/3 · @dos/offline 36 ·
@dos/ui 161 · @dos/api-client 63**, `format:check` clean in both workspaces, no `react-native` /
`react-dom` import outside the kit, `expo export --platform ios` ✓ (4.5 MB hbc) and `--platform
android` ✓ (4.8 MB) — then drove the harness in a real browser against the founder's own database and
the LIVE sales-service, at **1440 × 900 and 375 × 812**: sign in as `rahul.deshmukh`, **21 tables and
15 737 rows** land, go offline, queue an order, watch it appear at once marked _queued_, come back,
watch it drain exactly once, and **find the row in Postgres** (`sales_orders` `01a077…`, one row in
`sync_ops`). The retailer read-only path was driven against the live retailer-service on :3006 too:
18 tables, its own shop only, nothing writable, no cost column anywhere in the manifest.

**Seven defects found and fixed. The first one stopped the client dead after sixty seconds.**

1. **The device pulled ONCE per launch and then never again — and said "Offline" the whole time.**
   `status().online` also required a successful call inside the last 30 s, but the foreground poll runs
   every 60 s AND was itself gated on `online`: 30 s after the first pull the strip flipped to "Offline
   — saved on this phone", and the 60 s tick then saw `online === false`, skipped the sync, and
   rescheduled itself into the same dead state for ever. Every read-only app (owner, manager, retailer)
   would have shown a launch-time snapshot as if it were live, indefinitely. It was invisible because
   the harness set `pullIntervalMs={0}` — the one line that switched the defect off. `online` is now
   _the radio is on and the last call reached a service_; the poll is never gated on the state it
   produces (**the poll IS the probe**); the harness runs the real 60 s poll; docs/27 §10 is corrected;
   and a fake-timer test asserts a second pull after minute one. Walked: at +40 s "online true", at
   +75 s the cursor had moved.
2. **Coming back from a dead spot did not resync.** `setNetworkHint(_online)` threw its argument away
   and set `reachable = true` whatever the platform said — so on React Native, where there is no
   `navigator.onLine` to fall back on, the app could not say it was offline at all, and a reconnect
   started nothing. It now keeps the answer, clears the backoff, and pulls AND flushes on the rising
   edge; `<OfflineProvider>` wires the browser's own `online`/`offline` events to it.
3. **What the server made of a write never came back.** docs/27 §5 puts a pull "after every successful
   upload batch"; there was none, so a rep's screen kept the local copy of an order the server had
   priced, numbered and moved on.
4. **A shop's app would have shown a permanent error under a perfectly good sync.** `pullErrors` ran
   after every pull, and `transportFromApi` wires `sync.errors.list` for every app because the contract
   is shared — the live retailer-service answers **403 "the retailer role may not call GET
   /sync/errors"**, which threw AFTER the pull had committed and left that sentence on the strip. The
   tray mirror is best effort now, and a refusal stops the asking.
5. **Two distributors could share one device's tables.** `schemaVersion` is a hash of the ROLE's tables,
   so a rep who works for two distributors gets the same hash from both; nothing compared the tenant,
   and the second one's delta would land on the first one's rows. `SyncEngineOptions.tenantId` is
   stored in `_sync_state` and a change re-snapshots, with a test.
6. **`<MapView>` drew a beautiful map with NOT ONE PIN on it — in the production bundle as well.**
   maplibre-gl **6.7.0** starts its tiling worker from a blob that `import`s the worker chunk as an ES
   module, and in a bundled app that worker never runs: raster tiles come down on the main thread and
   look perfect, while every geojson source stays unloaded for ever with **no error event at all**
   (18 features in the source, 0 tiled, 0 rendered, empty console — proved with a textbook two-point
   circle layer as well, so it was not our layer code). Held at **5.24.0** in the catalog with the
   reason written next to it; all 18 shops render now. The component also learned `map.on('error')`,
   so the next silent failure is not silent.
7. **The map filled a third of its frame**, and refitted its bounds on every parent re-render (which
   would snatch the owner's live map back from under a pan). MapLibre measures its container once, at
   construction — a `ResizeObserver` now re-`resize()`s it, and the fit runs only when the PLACES
   change.

**Three design-system failures, measured rather than eyeballed.** Every shop name in the map's list
rendered in **Arial**: a `<button>` does not inherit the page font and `<Txt>` sets only the size
(founder, 2026-09-05: IBM Plex Sans everywhere). The harness's three header buttons ran **40 px off the
right edge at 375 px** — an unwrapped `<Row>` is one unbreakable flex item, so the header's own wrapping
had nothing to wrap. And it declared `touch="desk"`, which held every row of the map list at **32 px**
on a phone, under the 48 dp floor. After: 375 px `scrollWidth === clientWidth`, 0 targets under 44 px,
0 text under 12 px, every face IBM Plex Sans.

**Carried, for the founder, not blocking.**
(1) **No app consumes `@dos/offline` yet** — `owner-app` declares it, no screen imports it. `expo-sqlite`
and `react-native-maps` are optional peers of the libs and are declared by NO app, so autolinking does
not link them (`owner-app/ios/Podfile.lock` has neither): on a device today `openStore()` falls to the
memory adapter and the strip says so. The first field app must add `expo-sqlite` to its own
`package.json` and rebuild.
(2) **`owner-app` O4 Live map still says `MapView` "does not exist yet"** (`app/map.tsx:4`) and draws a
register only. It exists now; O4 can adopt it — and that adoption is where the map wants verifying
inside a METRO web build, which this gate could only verify through Vite (dev and production) because
no screen mounts one.
(3) **`expo run:ios` still cannot link on this Mac, and now `run:android` cannot either.** iOS:
`xcodebuild -showdestinations` lists NO eligible destination for the scheme — `iOS 18.2 is not
installed. To use with Xcode, first download and install the platform` — Xcode 16.2 ships the 18.2 SDK
and this machine has only the 18.0 simulator runtime. Android: `:react-native-worklets:configureCMake
Debug[arm64-v8a]` fails with `WARNING: A restricted method in java.lang.System has been called` —
Android Studio's bundled JDK is now **25.0.3**, too new for this Gradle/AGP. Both environment, neither
code; the native BUNDLES build for both platforms.
(4) The salesperson manifest marks only `sales_orders` and `sales_order_lines` writable, not the
thirteen tables docs/27 §1 lists. The manifest is the client's authority, so this is a note about the
server's `SyncRegistry` coverage, not a client defect.

**Local links.** Offline harness <http://localhost:5198> (`pnpm --filter @dos/offline harness` in
`frontend/`, or the `offline-harness` entry in `.claude/launch.json`) — needs auth-service :3000 and
sales-service :3003. Sign in **`rahul.deshmukh`** / `Dos@1234` (salesperson). Manager app
<http://127.0.0.1:5174> (`vikas.kadam`), owner app <http://127.0.0.1:5173> (`sunil.tarsun`) — every
password `Dos@1234`.

**NEXT: the sales app** (`frontend/sales-app`, docs/23 §3) — `pnpm --filter @dos/app-template new sales
5175`, then `pnpm docs:readme` in `backend/` in the SAME turn. It is the first FIELD app and the first
consumer of `@dos/offline`: add `expo-sqlite` to its own `package.json` (see carried item 1), mount
`<OfflineProvider>` inside the session gate, use the 69 dp touch floor, and register the four TTFs.

---

## Previously (updated 2026-09-06 20:30 IST, session 3)
...
| frontend slice F4 `@dos/offline` + kit `<MapView>` (docs/27, UX-00 §6.15) — our own delta sync client (memory / expo-sqlite adapters, manifest handshake, cursor pull loop, restart-proof outbox, GPS buffer, React layer) and the map component, walked in a real browser against the LIVE sales-service and retailer-service on the founder's own database at 1440 × 900 and 375 × 812. The independent gate fixed **seven defects plus three design-system failures**. The headline one stopped the client dead: `online` also required a call inside the last 30 s while the poll ran every 60 s AND was gated on `online`, so a device pulled **once per launch, then never again**, saying "Offline" the whole time — invisible because the harness had set `pullIntervalMs={0}`. Reconnect started nothing (`setNetworkHint` discarded its argument, so a React Native app could not even say it was offline); no pull followed a successful upload batch, so what the server made of a write never came back; a shop's app would have carried a permanent "the retailer role may not call GET /sync/errors" under a perfectly good sync (proved against :3006); and two distributors could share one device's tables because `schemaVersion` hashes the ROLE, not the tenant. `<MapView>` drew a perfect map with **not one pin** — maplibre-gl 6.7.0's ES-module blob worker never runs in a bundled app, so every geojson source stays unloaded with no error event (held at 5.24.0, with the reason in the catalog); it also filled a third of its frame (no `ResizeObserver`) and refitted its bounds on every re-render. Measured, not eyeballed: shop names rendered in **Arial** (`<button>` does not inherit the page font), three header buttons ran 40 px off a 375 px screen, and the map list sat at 32 px under the 48 dp floor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | lint 7/7 · typecheck 7/7 · build 4/4 · test 3/3 — **@dos/offline 36 · @dos/ui 161 · @dos/api-client 63** · `format:check` clean both workspaces · `--frozen-lockfile` clean · `expo export --platform ios` ✓ and `--platform android` ✓ · offline → queue → reconnect → drain proved end to end with the row in Postgres and ONE `sync_ops` row · 21 tables / 15 737 rows for the rep, 18 tables / own shop only for the retailer, **no cost or margin column in either manifest** · 375 px `scrollWidth === clientWidth`, 0 targets < 44 px, 0 text < 12 px, zero console output                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ✅ **GATE GREEN 2026-09-06 23:00** — web (Vite dev AND production bundle); native BUNDLES build for iOS and Android, but neither dev build links on this Mac (iOS 18.2 SDK vs 18.0 runtime; Android Studio's JDK 25 vs Gradle) — environment, not code. No app consumes the library yet: the sales app is the first.                                                                                                                     |
| frontend slice 4 `sales-app` (docs/23 §3) — the salesperson's app, the first FIELD app and the first consumer of `@dos/offline`: all **14 screens of §3.1** plus the four frame screens, walked screen by screen at **1440 × 900 and 375 × 812** against the founder's own database on the live auth :3000 and sales :3003. The independent gate fixed **six real defects, three of them in the shared kit**. The two that changed what a rep sees: the web `<Pressable>` rendered a ROW-direction flex `<button>`, which does not stretch a child along the main axis, so every list row was **195 px inside a 1168 px button** — money starting at x = 304 on a short row and x = 708 on a long one, no money column at all, and 973 px of every row dead to the touch (the native half is a View and always stretched, so the two renderers disagreed); and the beat computed each shop's last order from the newest **200** of the rep's orders, so on 442 rows of pilot data **six of the nine shops on the round read "No order yet"** while their own cards said "4 Sep". Also: "Last order" claimed a shop-wide fact from a device that holds only the rep's own orders (now "your last order" / "No order from you yet"); a **manager could sign in and was never refused** — the salesperson's shell around an empty beat and "Still loading the beat onto this phone" for ever while every call answered `403 sales-service does not serve the manager role` (the service's sentence is now the whole screen and the engine does not start); `<Segments>` was **65 dp against this app's 69 dp floor** on five phone screens; and a **stopped sync engine kept crawling the read set** — 530 `sync/pull` calls on one sign-in where the read set is 265 pages, latent in normal use and real on a sign-out or a distributor switch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | lint 8/8 · typecheck 8/8 · build 5/5 · test 3/3 — **@dos/offline 37 · @dos/ui 167 · @dos/api-client 63** · `format:check` clean · `--frozen-lockfile` clean · no `react-native` / `react-dom` import outside the kit · **18 desk screens + 12 phone screens with zero console errors and zero non-2xx** outside the role probe · device pricing == server pricing to the paisa (₹3,648.48 → `subtotal_paise 364848` on SO-1115) · offline → "Save on this phone" → tray "Waiting: 2" → reconnect → drains, the order on the server as the draft `orders.sync.ts` allows · service stopped → "Offline since 1:43 am", never a stale figure under "Updated just now" · manifest **21 tables, no cost / margin / landed column**, `tenantCatalog.costs` 403 · every phone screen: min font 14 px, IBM Plex Sans throughout, **0 targets under 69 dp**, no horizontal scroll                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅ **GATE GREEN 2026-09-07 03:15** — web + iOS (Expo Go, iPhone 16 Pro / 18.0: sign-in, beat, shop card, order entry); `expo run:android` **BUILD SUCCESSFUL on JDK 17** and the APK installs and launches, but the emulator then died and would not come back on this Mac (ephemeral console ports, `Failed to find ColorBuffer`) — environment, not code, and `expo run:ios` is still blocked by the 18.2 SDK / 18.0 runtime mismatch. |
| frontend slice 7 `retailer-app` (docs/23 §6) — the shopkeeper's app: online only, `field` touch, no tab bar, and the only one whose role may hold **more than one distributor** (`ramesh.gupta` buys from three). All **13 screens of §6.1** plus the frame screens, 16 routes walked at **1440 × 900 and 375 × 812** against the founder's own database on the live auth :3000 and retailer :3006, then on the **Pixel 7 / API 36** emulator from a real `expo run:android` build. The independent gate fixed **fourteen defects, five of them in the shared kit**. The three that would have cost a shop money or trust: after "Place order" — `setLines` 200, `submit` 200 — the screen the app navigates to read **"Not sent yet" with a live "Send this order" button** over an order the distributor had already accepted (the editor and the detail screen share `['order', id]`, and `['orders']` does not prefix it), and the confirmation toast was set after `router.replace` so it never drew; the bill screen's "WHAT YOU HAVE PAID" listed the shop's last eight receipts **whatever bill they were against** — on INV/0031, eight receipts totalling **minus ₹13,549**, none of them that bill's, and its own receipt missing; and "LAST BILL" named **INV/0031 of 27 Aug** while the shop's last bill was INV/9125 of 5 Sep, because `invoices.list` pages by `id DESC`. Also: My bills, My orders and the **inbox** in no date order at all; the account screen listing **41 devices with no date on any of them** (`<Register>` drops every `detail` column in `field` density); "1 items"; the statement calling a **write-off a payment** and a reversed receipt a bill; "Sent by Sms" and "Sent by On WhatsApp"; a **uuid printed as the shop's own note**; and the order screen opening on an empty text box. Kit: the desk rail **cut the distributor's own name through a letter** ("Tarsun Enterpris") because a field app draws the rail at `field.title` 20 px in an 88 px clipped box; `<TenantSwitcher>` sized the mark by the app instead of the shell; the switcher itself — this app's spine — was a **52 dp** target against a 69 dp floor; and a `<Segments>` item was **54 dp** wide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | lint 11/11 · typecheck 11/11 · build 8/8 · test 3/3 — **@dos/ui 196 · @dos/api-client 63 · @dos/offline 38** · `format:check` clean in both workspaces · `docs:readme:check` clean · lockfile unchanged by install · native-resolution `tsc --noEmit` clean · no `react-native` / `react-dom` import and no raw hex in any app file · **16 desk routes + 16 phone routes with zero console errors, zero warnings, zero non-2xx, zero horizontal scroll and not one tap target under its floor** · two-tap reorder → **SO-0075 on the server**, the detail screen reading "Accepted" · distributor switch re-scopes shop, dues, orders, bills and chrome, with each distributor's own mark · :3006 blocked → every figure an em dash under "Offline since 5:44 pm" · an owner is refused with the service's own sentence and **0 calls to :3006** · no cost / margin / landed word, no snake_case and no uuid on any screen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ **GATE GREEN 2026-09-07 17:45** — web (desk + phone shells) and **Android** (Pixel 7, `expo run:android` BUILD SUCCESSFUL **on JDK 21, installed by this gate** — Android Studio's bundled JDK 25 makes AGP treat its own stderr warning as a prefab error). iOS boots and renders in Expo Go on the iPhone 16 Pro / 18.0 and still cannot be signed in headlessly on this Mac — environment, not code.                               |
| frontend slice 6 `delivery-app` (docs/23 §5) — the crew's app, the first with **no tab bar** (UX-00 §8.2: a single stack that opens on the next stop) and the first to write at a shop door: all **12 screens of §5.1** plus the frame screens, 16 routes walked at **1440 × 900 and 375 × 812** against the founder's own database on the live auth :3000 and delivery :3005, then on the **Pixel 7 / API 36** emulator from a real `expo run:android` build. The independent gate fixed **ten defects, four of them outside this app**. The three that would have cost a driver something real: `@dos/offline`'s `queryTable` answers `[]` for a table whose shape is not loaded yet and `useTable` re-runs only on a table CHANGE, so on a phone whose store is already full and whose delta pull brings nothing new **the home screen said "Nothing is on the road yet" on every cold start** over a SQLite file (pulled off the device) holding TRIP-NEXT as `active` — the offline-first app failing exactly where it exists to work, and latent in the sales and warehouse apps too; the day summary added up the office's `collections` while the phone held **₹2,500 of receipts the office had not counted**, and told the driver to hand over ₹11,070 instead of ₹13,570; and `/` named TRIP-NEXT while `/trip/start` and `/expenses` named TRIP-ACTIVE for the whole first pull — a live "Start the trip" under the wrong trip and an expense form pointed at yesterday's. Also: "Van sales are switched off for this trip" over a trip that had simply not arrived; "Stop 3 of **3**" on a six-stop day; "Your trips" unwindowed and unordered under a "Last 30 days" chip, without the crew's own active trip, **every row opening today's trip**; Android's `ACCESS_BACKGROUND_LOCATION` rejection escaping as a red box over the home screen with no tracking at all; a D1 sentence promising background tracking nothing in this repo does; and the phone's only menu listing **Me and Trip history twice**. Kit: `<ConnectionStrip>` dropped the pending count while OFFLINE — the one state where unsent work matters — and `<RupeeInput>`'s 69 dp control had a 67 dp `<input>` inside its border.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | lint 10/10 · typecheck 10/10 · build 7/7 · test 3/3 — **@dos/ui 190 · @dos/api-client 63 · @dos/offline 38** · `format:check` clean · `--frozen-lockfile` clean · native-resolution `tsc --noEmit` clean · no `react-native` / `react-dom` import outside the kit · **16 desk routes + 16 phone routes with zero console errors, zero warnings and zero non-2xx** · offline arrival → tray "Waiting: 1" → reconnect → **`trip_stops.state = 'arrived'` on the server with the driver's own 12:01:19 tap time** · service dead → "Offline since 12:04 pm", never a stale figure under "Updated just now" · a manager is refused with the service's own sentence and **0 further calls** · no cost / margin / landed field in any DOM or payload · phone: **no tab bar**, min font 14 px, IBM Plex Sans, no horizontal scroll at 375 px                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ **GATE GREEN 2026-09-07 14:10** — web (desk + phone shells) and **Android** (Pixel 7, `expo run:android` BUILD SUCCESSFUL on JDK 17, `adb`-driven sign-in and five screens, zero `ReactNativeJS` errors). iOS boots and renders in Expo Go on the iPhone 16 Pro / 18.0 and still cannot be signed in headlessly on this Mac — environment, not code.                                                                                  |
| frontend slice 5 `warehouse-app` (docs/23 §4) — the godown's app at the **76 dp** floor: all **12 screens of §4.1** plus the four frame screens, 20 routes walked at **1440 × 900 and 375 × 812** against the founder's own database on the live auth :3000 and warehouse :3004, then headlessly on the **Pixel 7 / API 36** emulator. The independent gate fixed **eight real defects, five of them in the shared kit**. The three that changed what a hand sees: `LocalAsync` drew "Still filling this phone from the server" INSTEAD of the device's rows, so the picking sheet counted **"1 of 23 picked" twice and drew none of the 23** (and on the web, where the offline store is the memory adapter, that is 154 pull pages and ~70 s after every reload); the van check-in printed `caseLine(pcs, 1)`, so **201 pieces of a 48-piece case read "201 cs = 201 pc"** to the loader counting them back; and the variant-name index capped `product_variants` at **5,000 rows against 5,768**, so the gate count's title read "Item not on this phone yet" over "Too Yumm Makhana Himalayan Salt 20 g" — names are read BY ID now. Kit: `<NumberPad>`'s action button fell through to the browser's **13.33 px** default under both blind counts; its `expected` was drawn as `<Money>` in `count` mode (**₹2.01 for 201 pieces**); the web `<Chips>` shipped a **9.5 px** gap where the native half shipped 25 (UX-00 §6.10), fixed with the pad's key grid, the bottom bar's clearance from the tab bar and this app's own two-button rows; and Android drew its **status bar in white on a white app** (`expo-status-bar` was a dependency of all seven apps and used by none — fixed in the template too). Plus a **401 on every cold start**, the sync engine starting before the access token existed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | lint 9/9 · typecheck 9/9 · build 6/6 · test 3/3 — **@dos/ui 184 · @dos/api-client 63 · @dos/offline 37** · `format:check` clean · lockfile unchanged by install · no `react-native` / `react-dom` import outside the kit · **20 desk routes + 20 phone routes with zero console errors, zero warnings and zero non-2xx** · offline pick → outbox → **`pick_lines.picked_qty_pcs = 24` on the server** · service stopped → "Offline since 8:52 am", every KPI an em dash, no stale figure under "Updated just now" · a manager is refused with the service's own sentence and **0 further calls** · `tenantCatalog.costs` and `stock-value` both 403, the warehouse manifest 13 tables with **no cost / margin / landed column** · every screen: min font 14 px, IBM Plex Sans, no horizontal scroll at 375 px                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ✅ **GATE GREEN 2026-09-07 09:10** — web (desk + phone shells) and **Android** (Pixel 7, `adb`-driven sign-in and the five key screens). iOS boots and renders in Expo Go on the iPhone 16 Pro / 18.0 but cannot be signed in headlessly on this Mac: `simctl` has no input verb, `idb-companion` needs macOS 26 / Xcode 26, and the panel is off limits (founder, 2026-09-06) — environment, not code.                                  |
