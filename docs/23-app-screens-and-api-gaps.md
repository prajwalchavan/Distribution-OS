...
- **two of the three demo distributors carry a trial that ended on 25/26 January 2018** under a subscription still reading
  `trial`, so the console shows "Trial ended 25 Jan 2018" beside "On trial". `seedPlatformConsole` writes
  `trialEndsAt = Date.now() + (20 - index) * DAY_MS` over only the three demo slugs today, but the rows in the founder's
  database were written by an older seed that mapped over EVERY tenant (`index ≈ 3165`, i.e. 3 145 days in the past), and
  `insertMany(..., onConflictDoNothing())` means `pnpm db:seed` can never correct a row it already wrote. Either the seed
  upserts these four columns or the two rows are deleted once by hand. (A distributorship onboarded from the console today
  gets a correct date — proved: `frontgate-traders-333914`, trial ends 2026-10-07.)
